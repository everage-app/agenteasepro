...
import { commonStack, pricingDisclaimer } from '../../config/comparisonData';
  ...
  return (
    <section id="pricing" className="relative py-20 md:py-32 overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="rounded-3xl border border-white/[0.08] bg-slate-900/55 p-8 md:p-10 backdrop-blur-xl">
          <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-[#f4b860]/20 bg-[#f4b860]/10 px-4 py-2 text-sm font-medium text-[#f4b860] mb-5">
                Compare the real starting point
              </div>
              <h3 className="text-2xl md:text-3xl font-bold text-white mb-4">
                Most competitor prices are <span className="gradient-text">base plans, not full workflows.</span>
              </h3>
              <p className="text-slate-400 leading-relaxed mb-6">
                Public starting pricing for basic CRM plus website functionality already reaches {commonStack.total}. That still leaves contracts, e-signatures, Utah REPC workflow, deadline automation, and compliance outside the package.
              </p>

              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                <Link
                  to="/compare"
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-white/[0.04] border border-white/10 px-6 py-3 font-semibold text-white transition-all duration-300 hover:border-[#f4b860]/30 hover:text-[#f4b860]"
                >
            ...
            <div className="rounded-3xl border border-[#f4b860]/20 bg-gradient-to-br from-[#f4b860]/12 to-amber-500/10 p-6 md:p-7">
              <div className="text-sm font-semibold uppercase tracking-[0.18em] text-[#f4b860]">AgentEasePro</div>
              <div className="mt-2 text-4xl font-bold text-white">$49.99/mo</div>
              <p className="mt-3 text-sm leading-relaxed text-slate-200">
                Full workflow included. No per-user math. No base-plan bait. No patching together separate tools just to operate.
              </p>

              <div className="mt-5 rounded-2xl border border-white/[0.08] bg-slate-950/70 px-5 py-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <div className="text-sm text-slate-400">Public starting point elsewhere</div>
                    <div className="text-xs text-slate-500">Basic CRM + website pricing only</div>
                  </div>
                  <div className="text-2xl font-bold text-white">{commonStack.total}</div>
                </div>
              </div>

              <div className="mt-5 flex items-center justify-between gap-4 rounded-2xl border border-white/[0.08] bg-black/20 px-5 py-4">
                <div>
                  <div className="text-sm text-slate-400">Monthly savings</div>
                  <div className="text-xs text-slate-500">Against the public CRM + website starting point</div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-white">{commonStack.savings}</div>
                  <div className="text-sm font-medium text-[#f4b860]">{commonStack.savingsPercent} less</div>
                </div>
              </div>

              <p className="mt-5 text-xs leading-relaxed text-slate-300/80">{pricingDisclaimer}</p>
            </div>
          </div>
        </div>
      </div>
