  ...
  const steps = [
    {
      ...
      description: "Create your account with no card up front. Import your existing clients or start fresh - we make onboarding a breeze.",
      ...
      features: ["No card up front", "Import existing clients", "Utah-ready templates"],
    },
    {
      number: "02",
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
          ...
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-6">
            {steps.map((step, index) => (
              <div key={index} className="relative group">
                {/* Step card */}
                <div className="relative h-full rounded-2xl p-7 md:p-8 bg-gradient-to-br from-slate-900/90 to-slate-800/50 border border-white/[0.06] backdrop-blur-xl transition-all duration-500 hover:border-white/20 hover:scale-[1.02]"
                  style={{
                    boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5), 0 0 0 1px rgba(255, 255, 255, 0.03) inset'
                  }}
                >
                  {/* Number badge inside the square tile */}
                  <div className={`mb-7 w-16 h-16 rounded-2xl bg-gradient-to-br ${step.color}/10 border border-white/10 flex items-center justify-center`}>
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${step.color} flex items-center justify-center font-bold text-lg leading-none text-slate-900`}>
                      {step.number}
                    </div>
                  </div>

                  {/* Content */}
                  <h3 className="text-xl font-bold text-white mb-3 min-h-[3.5rem]">{step.title}</h3>
        ...
        <div className="mt-16 text-center">
          ...
          <p className="mt-4 text-sm text-slate-500">
            No card to create an account • Cancel anytime
          </p>
        </div>
      </div>
    </section>
