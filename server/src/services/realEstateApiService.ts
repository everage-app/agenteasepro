...
export async function searchProperties(agentId: string, params: SearchParams): Promise<PropertyResult[]> {
  ...
  // If no real results found, provide sample data in development/demo mode only.
  if (unique.length === 0) {
    const sampleDataEnabled = process.env.ENABLE_PROPERTY_SAMPLE_DATA === 'true' || process.env.NODE_ENV !== 'production';
    if (sampleDataEnabled) {
      ...
      console.log('[PropertySearch] No verified property results found; sample data disabled');
    }
  }
