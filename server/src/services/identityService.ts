...
export async function signup(
  ...
  if (existing) {
    ...
    return { success: false, error: 'Unable to create account. Please try again or use the forgot password option.' };
  }
  ...
  const agent = await prisma.agent.create({
    data: {
      ...
      passwordHash,
      emailVerified: false,
      emailVerifyToken: hashToken(verifyCode),
...
/** SHA-256 hash a token before persisting — store the hash, compare by re-hashing. */
...
export async function verifyEmail(
  ...
  // Compare by hashing the submitted code and using timing-safe comparison
  const codeHashBuffer = Buffer.from(hashToken(code.trim()));
  const storedBuffer = Buffer.from(agent.emailVerifyToken);
  if (codeHashBuffer.length !== storedBuffer.length || !crypto.timingSafeEqual(codeHashBuffer, storedBuffer)) {
    await auditLog({
      action: 'AUTH_EMAIL_VERIFY_FAILED',
      agentId: agent.id,
      email: agent.email,
...
export async function resendVerificationCode(
  ...
  await prisma.agent.update({
    where: { id: agent.id },
    data: {
      emailVerifyToken: hashToken(newCode),
