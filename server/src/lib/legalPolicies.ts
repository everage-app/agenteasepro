...
export async function getLegalPolicies(): Promise<LegalPolicies> {
  await prisma.$executeRaw(Prisma.sql`
    INSERT INTO "LegalPolicy" ("policyKey", "url", "version", "updatedAt")
    VALUES ('terms', ${DEFAULT_TERMS_URL}, ${DEFAULT_VERSION}, CURRENT_TIMESTAMP)
    ON CONFLICT ("policyKey") DO NOTHING
  `);

  await prisma.$executeRaw(Prisma.sql`
    INSERT INTO "LegalPolicy" ("policyKey", "url", "version", "updatedAt")
    VALUES ('privacy', ${DEFAULT_PRIVACY_URL}, ${DEFAULT_VERSION}, CURRENT_TIMESTAMP)
    ON CONFLICT ("policyKey") DO NOTHING
  `);
...
export async function updateLegalPolicies(
  ...
  await prisma.$executeRaw(Prisma.sql`
    INSERT INTO "LegalPolicy" ("policyKey", "url", "version", "updatedAt", "updatedByAgentId", "updatedByAgentName")
    VALUES ('terms', ${termsUrl}, ${termsVersion}, CURRENT_TIMESTAMP, ${updater?.id || null}, ${updater?.name || null})
  ...
  await prisma.$executeRaw(Prisma.sql`
    INSERT INTO "LegalPolicy" ("policyKey", "url", "version", "updatedAt", "updatedByAgentId", "updatedByAgentName")
    VALUES ('privacy', ${privacyUrl}, ${privacyVersion}, CURRENT_TIMESTAMP, ${updater?.id || null}, ${updater?.name || null})
    ON CONFLICT ("policyKey")
    DO UPDATE SET
      "url" = EXCLUDED."url",
      "version" = EXCLUDED."version",
