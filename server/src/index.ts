...
// Note: this repo keeps shared environment variables at the monorepo root.
// Shell-provided values should always win, while server/.env should beat the
// root file for local server defaults like PORT or a local DATABASE_URL.
const originalEnvKeys = new Set(Object.keys(process.env));
const rootEnvPath = path.join(__dirname, '../../.env');
const serverEnvPath = path.join(__dirname, '../.env');

const isPlaceholderEnvValue = (value: string) => /YOUR_PASSWORD|CHANGEME|REPLACE_ME|<[^>]+>/i.test(value);
...
if (fs.existsSync(serverEnvPath)) {
  ...
  for (const [key, value] of Object.entries(serverEnv)) {
    if (!originalEnvKeys.has(key) && !isPlaceholderEnvValue(value)) {
...
import { telnyxWebhookHandler } from './routes/telnyxWebhook';
import { router as oauthRouter } from './routes/oauth';
import { router as contactEmailRouter } from './routes/contactEmail';
import { router as contactSmsRouter } from './routes/contactSms';
import { router as dealStakeholdersRouter } from './routes/dealStakeholders';
import { router as routingRulesRouter } from './routes/routingRules';
import webhooksRouter from './routes/webhooks';
import esignAuditsRouter from './routes/esignAudits';
import { prisma } from './lib/prisma';
import { errorHandler } from './lib/apiResponse';

const app = express();

// Heroku runs behind a reverse proxy — trust the first proxy hop so that
// req.ip, rate-limiter keys, and audit-log IPs reflect the real client.
...
// Telnyx Webhook for incoming SMS and delivery status updates
app.post(
  '/api/integrations/telnyx/events',
  express.raw({ type: 'application/json' }),
...
app.use('/api/deals/:dealId/stakeholders', authMiddleware, dealStakeholdersRouter);
app.use('/api/routing-rules', authMiddleware, routingRulesRouter);
app.use('/api/webhooks', webhooksRouter);
app.use('/api/esign-audits', esignAuditsRouter);
