...
const S3_MANAGED_URL_RE = /^https:\/\/[^/]+\.s3(?:[.-][^/]+)?\.amazonaws\.com\//i;

const shouldInlineSettingsImages = () => {
  const forceInline = (process.env.INLINE_SETTINGS_IMAGES || '').toLowerCase();
...
const toInlineImageUrl = (file: Express.Multer.File) => {
  const mimeType = file.mimetype || 'image/png';
  return `data:${mimeType};base64,${file.buffer.toString('base64')}`;
};

const persistSettingsImage = async (
  file: Express.Multer.File,
  agentId: string,
  folder: string,
  prefix: string,
) => {
  if (shouldInlineSettingsImages()) {
    return toInlineImageUrl(file);
  }

  const ext = resolveImageExtension(file);
  const key = `${folder}/${prefix}-${agentId}-${Date.now()}${ext}`;
  const saved = await storage.put(key, file.buffer, file.mimetype || 'application/octet-stream');
  return saved.url;
};

const extractManagedStorageKey = (assetUrl?: string | null): string | null => {
  if (!assetUrl) return null;

  const normalized = assetUrl.split('#')[0].split('?')[0].trim();
  ...
  if (cdnBase && normalized.startsWith(`${cdnBase}/`)) {
    return normalized.slice(cdnBase.length + 1);
  }
  ...
  try {
    ...
    if (S3_MANAGED_URL_RE.test(normalized)) {
      return pathname;
    }
...
const deleteManagedAsset = async (assetUrl: string | null | undefined, label: string) => {
  const key = extractManagedStorageKey(assetUrl);
  if (!key) return;

  try {
    await storage.delete(key);
  } catch (err) {
    console.warn(`Unable to delete ${label}:`, err);
  }
};
...
router.put('/profile', async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const agentUpdates: { name?: string; brokerageName?: string | null; licenseNumber?: string | null } = {};

    if (firstName || lastName) {
      agentUpdates.name = [firstName, lastName].filter(Boolean).join(' ');
    }

    if (brokerageName !== undefined) {
      agentUpdates.brokerageName = brokerageName || null;
    }

    if (licenseNumber !== undefined) {
      agentUpdates.licenseNumber = licenseNumber || null;
    }
    ...
    const parsedBrokerageLogoWidth = Number(brokerageLogoWidth);
    const normalizedBrokerageLogoWidth = Number.isFinite(parsedBrokerageLogoWidth)
      ? Math.min(420, Math.max(140, Math.round(parsedBrokerageLogoWidth)))
      : undefined;
    const hasBrokerageLogoBackground = brokerageLogoBackground === 'TRANSPARENT' || brokerageLogoBackground === 'CARD';
    const normalizedBrokerageLogoBackground = brokerageLogoBackground === 'TRANSPARENT' ? 'TRANSPARENT' : 'CARD';
...
router.post('/branding/logo', handleLogoUpload, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const logoUrl = await persistSettingsImage(req.file, req.agentId, 'logos', 'logo');
...
router.post('/profile/brokerage-logo', handleLogoUpload, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const brokerageLogoUrl = await persistSettingsImage(req.file, req.agentId, 'logos', 'brokerage');
...
router.post('/profile/photo', handleProfilePhotoUpload, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const photoUrl = await persistSettingsImage(req.file, req.agentId, 'profiles', 'profile');
...
router.delete('/branding/logo', async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    await prisma.agentProfileSettings.upsert({
      where: { agentId: req.agentId },
      create: { agentId: req.agentId, logoUrl: null },
      update: { logoUrl: null }
    });
...
router.get('/notifications/feed', async (req: AuthenticatedRequest, res) => {
  if (!req.agentId) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  ...
  try {
    ...
    const items = events.map((event) => {
      ...
      return {
        id: event.id,
        at: event.createdAt,
        leadId: typeof meta.leadId === 'string' ? meta.leadId : '',
        leadName: typeof meta.leadName === 'string' ? meta.leadName : 'New lead',
        leadEmail: typeof meta.leadEmail === 'string' ? meta.leadEmail : null,
        leadPhone: typeof meta.leadPhone === 'string' ? meta.leadPhone : null,
        sourceLabel: typeof meta.sourceLabel === 'string' ? meta.sourceLabel : null,
        landingPageId: typeof meta.landingPageId === 'string' ? meta.landingPageId : null,
