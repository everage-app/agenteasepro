...
import { buildLeadOpsTags, getLeadOpsSettings, saveLeadOpsSettings } from '../services/leadOpsSettings';
...
const normalizeMailingAddressFields = (source?: {
  mailingAddress?: string | null;
  mailingCity?: string | null;
  mailingState?: string | null;
  mailingZip?: string | null;
}) => ({
  mailingAddress: normalizeOptionalText(source?.mailingAddress),
  mailingCity: normalizeOptionalText(source?.mailingCity),
  mailingState: normalizeOptionalText(source?.mailingState),
  mailingZip: normalizeOptionalText(source?.mailingZip),
});
...
const leadCreateSchema = z
  .object({
    ...
    mailingCity: z.string().trim().max(120).optional(),
    mailingState: z.string().trim().max(40).optional(),
...
const leadUpdateSchema = z
  .object({
    ...
    mailingCity: z.string().trim().max(120).nullable().optional(),
    mailingState: z.string().trim().max(40).nullable().optional(),
...
const routingSlaSettingsSchema = z
  .object({
    routingMode: z.enum(['MANUAL', 'ASSIGN_TO_SELF', 'LABEL']).default('ASSIGN_TO_SELF'),
    assignToSelf: z.boolean().optional(),
    assignToLabel: z.preprocess(
      (value) => value == null ? undefined : value,
      z.string().trim().max(120).optional(),
    ),
    followUpEnabled: z.boolean().default(true),
    followUpMinutes: z.number().int().min(5).max(1440).default(15),
    warningMinutes: z.number().int().min(5).max(1440).default(10),
    escalationAction: z.enum(['TASK_ONLY', 'NOTIFY_AGENT']).default('TASK_ONLY'),
  })
  .strict();
...
router.get('/smart-lists', async (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/smart-lists', async (req: AuthenticatedRequest, res) => {
  try {
...
router.delete('/smart-lists/:id', async (req: AuthenticatedRequest, res) => {
  try {
...
router.get('/settings/routing-sla', async (req: AuthenticatedRequest, res) => {
  try {
    const agentId = req.user!.id;
    const [settings, openLeadCount, unassignedLeadCount, overdueFirstResponseCount] = await Promise.all([
      getLeadOpsSettings(agentId),
      prisma.lead.count({
        where: {
          agentId,
          converted: false,
          deletedAt: null,
          NOT: { tags: { has: ARCHIVED_TAG } },
        },
      }),
      prisma.lead.count({
        where: {
      ...
      prisma.task.count({
        where: {
          ...
          createdFrom: 'SYSTEM',
          dueAt: { lt: new Date() },
          title: { startsWith: 'First response:' },
        },
      }),
    ]);
    ...
    res.json({
      ...
      readiness: {
        openLeadCount,
        unassignedLeadCount,
        overdueFirstResponseCount,
      },
    });
  } catch (error) {
    console.error('Error fetching lead routing/SLA settings:', error);
    res.status(500).json({ error: 'Failed to fetch lead routing/SLA settings' });
  }
});

router.put('/settings/routing-sla', async (req: AuthenticatedRequest, res) => {
  try {
    const parsed = routingSlaSettingsSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: 'Invalid routing/SLA settings' });
    }

    const settings = await saveLeadOpsSettings(req.user!.id, {
      ...parsed.data,
      assignToSelf: parsed.data.routingMode === 'ASSIGN_TO_SELF' || Boolean(parsed.data.assignToSelf),
    });
    res.json(settings);
  } catch (error) {
    console.error('Error saving lead routing/SLA settings:', error);
    res.status(500).json({ error: 'Failed to save lead routing/SLA settings' });
  }
});
  ...
  try {
    ...
    for (let i = 0; i < unique.length; i += CHUNK) {
      ...
      for (const r of chunk) {
        ...
        const normalizedMailing = normalizeMailingAddressFields(r);
        ...
        if (existingLead) {
          ...
          if (!existingLead.mailingAddress && normalizedMailing.mailingAddress) nextData.mailingAddress = normalizedMailing.mailingAddress;
          if (!existingLead.mailingCity && normalizedMailing.mailingCity) nextData.mailingCity = normalizedMailing.mailingCity;
          if (!existingLead.mailingState && normalizedMailing.mailingState) nextData.mailingState = normalizedMailing.mailingState;
          if (!existingLead.mailingZip && normalizedMailing.mailingZip) nextData.mailingZip = normalizedMailing.mailingZip;
          ...
          if (normalizedNotes) {
            ...
            if (!existingNotes.includes(normalizedNotes)) {
              nextData.notes = existingNotes ? `${existingNotes}\n\n${normalizedNotes}` : normalizedNotes;
            }
          }
        ...
        toCreate.push({
          ...
          mailingCity: normalizedMailing.mailingCity,
          mailingState: normalizedMailing.mailingState,
          mailingZip: normalizedMailing.mailingZip,
          source,
          priority,
    ...
    const where: any = {
      ...
      OR: search
        ? [
            ...
            { mailingAddress: { contains: search as string, mode: 'insensitive' } },
            { mailingCity: { contains: search as string, mode: 'insensitive' } },
            { mailingState: { contains: search as string, mode: 'insensitive' } },
            { mailingZip: { contains: search as string, mode: 'insensitive' } },
          ]
        : undefined,
    };
...
router.post('/:id/convert', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const resolvedMailing = {
      mailingAddress: normalizeOptionalText(typeof req.body?.mailingAddress === 'string' ? req.body.mailingAddress : lead.mailingAddress),
      mailingCity: normalizeOptionalText(typeof req.body?.mailingCity === 'string' ? req.body.mailingCity : lead.mailingCity),
      mailingState: normalizeOptionalText(typeof req.body?.mailingState === 'string' ? req.body.mailingState : lead.mailingState),
      mailingZip: normalizeOptionalText(typeof req.body?.mailingZip === 'string' ? req.body.mailingZip : lead.mailingZip),
    };
    ...
    const result = await prisma.$transaction(async (tx) => {
      const client = await tx.client.create({
        data: {
          ...
          mailingAddress: resolvedMailing.mailingAddress,
          mailingCity: resolvedMailing.mailingCity,
          mailingState: resolvedMailing.mailingState,
          mailingZip: resolvedMailing.mailingZip,
          tags: lead.tags || [],
        },
      });
      ...
      const updatedLead = await tx.lead.update({
        ...
        data: {
          ...
          mailingAddress: resolvedMailing.mailingAddress || null,
          mailingCity: resolvedMailing.mailingCity || null,
          mailingState: resolvedMailing.mailingState || null,
          mailingZip: resolvedMailing.mailingZip || null,
        },
...
router.post('/', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const effectiveSource = parsedSource ?? LeadSource.WEBSITE;
    ...
    if (existingLead) {
      ...
      const updated = await prisma.lead.update({
        ...
        data: {
          ...
          mailingAddress: existingLead.mailingAddress || normalizedMailing.mailingAddress,
          mailingCity: existingLead.mailingCity || normalizedMailing.mailingCity,
          mailingState: existingLead.mailingState || normalizedMailing.mailingState,
          mailingZip: existingLead.mailingZip || normalizedMailing.mailingZip,
          listingId: listingId || existingLead.listingId,
          landingPageId: landingPageId || existingLead.landingPageId,
          source: isLandingCapture ? LeadSource.LANDING_PAGE : (parsedSource || existingLead.source),
    ...
    await prisma.leadActivity.create({
      data: {
        ...
        metadata: {
          ...(utmData || {}),
          routingMode: leadOps.routingMode,
          firstResponseMinutes: leadOps.followUpMinutes,
        },
      },
    });

    if (leadOps.followUpEnabled) {
      await runLeadCaptureWorkflow({
        ...
        sourceLabel: isLandingCapture ? 'landing page' : effectiveSource.toLowerCase().replace(/_/g, ' '),
        message: notes || null,
        contextPath: '/api/leads',
        listingId,
        landingPageId,
        createTask: true,
        followUpMinutes: leadOps.followUpMinutes,
        taskDescription: `First response SLA: contact this lead within ${leadOps.followUpMinutes} minutes.`,
      });
    }
  ...
  try {
    ...
    const lead = await prisma.lead.update({
      ...
      data: {
        ...
        ...(mailingAddress !== undefined && { mailingAddress: normalizedMailing.mailingAddress ?? null }),
        ...(mailingCity !== undefined && { mailingCity: normalizedMailing.mailingCity ?? null }),
        ...(mailingState !== undefined && { mailingState: normalizedMailing.mailingState ?? null }),
        ...(mailingZip !== undefined && { mailingZip: normalizedMailing.mailingZip ?? null }),
