...
import { isOwnerRequest, requireInternal } from '../middleware/requireOwner';
...
import {
  ...
  buildOwnerBriefing,
  getInternalAccessForRequest,
  listInternalAuditLogs,
  listInternalStaff,
  roleCanManage,
  supportDueAtForPriority,
  upsertInternalStaff,
  writeInternalAudit,
  type InternalRole,
} from '../services/internalOpsService';
...
const staffRoleSchema = z.enum(['OWNER', 'ADMIN', 'SUPPORT', 'BILLING', 'SALES', 'PRODUCT', 'ENGINEERING', 'READ_ONLY']);
...
async function requireInternalCapability(
  req: AuthenticatedRequest,
  res: { status: (code: number) => any; json: (body: any) => any },
  capability: 'staff' | 'billing' | 'support' | 'sales' | 'system',
  message: string,
) {
  const access = await getInternalAccessForRequest(req);
  if (!roleCanManage(access.role as InternalRole | null, capability)) {
...
router.get('/access', async (req: AuthenticatedRequest, res) => {
  const access = await getInternalAccessForRequest(req);
  const ownerAllowed = await isOwnerRequest(req);
  return res.json({
...
// Everything below requires approved internal access.
router.use(requireInternal);

router.get('/ops/briefing', async (_req, res) => {
  const briefing = await buildOwnerBriefing();
  return res.json(briefing);
});

router.get('/ops/agent-health', async (req, res) => {
  const take = Math.min(Math.max(Number(req.query.take || 100), 10), 500);
  const includeArchived = String(req.query.includeArchived || '').toLowerCase() === 'true';
  const profiles = await buildAgentHealthProfiles({ take, includeArchived });
  return res.json({ profiles });
});

router.get('/ops/audit-log', async (req, res) => {
  if (!(await requireInternalCapability(req as AuthenticatedRequest, res, 'system', 'Only owner/admin/engineering can view audit logs'))) return;

  const pagination = parsePagination(req, res);
  if (!pagination) return;
  const q = z.string().trim().max(120).optional().parse(req.query.q);
  const action = z.string().trim().max(80).optional().parse(req.query.action);
...
router.get('/ops/staff', async (_req, res) => {
  const staff = await listInternalStaff();
  return res.json({ staff });
});

router.put('/ops/staff', async (req: AuthenticatedRequest, res) => {
  const access = await getInternalAccessForRequest(req);
  if (!roleCanManage(access.role as InternalRole | null, 'staff')) {
    return res.status(403).json({ error: 'Only owner/admin can manage internal staff' });
  }

  const parsed = staffUpsertSchema.safeParse(req.body || {});
  if (!parsed.success) {
    return res.status(400).json({ error: 'Invalid staff payload', details: parsed.error.flatten() });
  }
  ...
  if (before?.role === 'OWNER' && !access.isMasterOwner) {
    return res.status(403).json({ error: 'Only the master owner can change an OWNER staff record' });
  }
  ...
  await writeInternalAudit({
    ...
    summary: `${staff.agent.email} set to ${staff.role}${staff.active ? '' : ' (inactive)'}`,
    before,
    after: staff,
...
router.post('/campaigns/preview', async (req: AuthenticatedRequest, res) => {
  ...
  if (!(await requireInternalCapability(req, res, 'sales', 'Only sales/admin staff can preview internal campaigns'))) return;
...
router.post('/campaigns/send', async (req: AuthenticatedRequest, res) => {
  if (!req.agentId) return res.status(401).json({ error: 'Unauthorized' });
  if (!(await requireInternalCapability(req, res, 'sales', 'Only sales/admin staff can send internal campaigns'))) return;
  ...
  if (recentLaunch) {
    ...
    if (blast) {
      await writeInternalAudit({
        req,
        action: 'campaign.deduplicated',
        targetType: 'marketingBlast',
        targetId: blast.id,
        summary: `Deduplicated internal campaign launch for ${parsed.data.subject}`,
        after: { campaignId: blast.id, subject: parsed.data.subject, recipients: deliverable.length },
      });
  ...
  await writeInternalAudit({
    ...
    targetType: 'marketingBlast',
    targetId: blast.id,
    summary: `Internal campaign queued to ${deliverable.length} recipient(s): ${parsed.data.subject}`,
    after: {
      campaignId: blast.id,
      subject: parsed.data.subject,
      totalParsed: extracted.valid.length,
...
router.get('/campaigns/:id/status', async (req: AuthenticatedRequest, res) => {
  if (!req.agentId) return res.status(401).json({ error: 'Unauthorized' });
  if (!(await requireInternalCapability(req, res, 'sales', 'Only sales/admin staff can view internal campaign status'))) return;
...
router.get('/agents', async (req, res) => {
  ...
  const healthProfiles = agents.length
    ? await buildAgentHealthProfiles({ agentIds: agents.map((agent) => agent.id), includeArchived: true })
    : [];
  const healthByAgentId = new Map(healthProfiles.map((profile) => [profile.agent.id, profile]));
  ...
  return res.json({
    ...
    agents: agents.map((agent) => ({
      ...agent,
      health: healthByAgentId.get(agent.id) || null,
    })),
  });
});

router.post('/agents/bulk-status', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'staff', 'Only owner/admin can bulk update agent status'))) return;
  ...
  const before = await prisma.agent.findMany({
  ...
  await writeInternalAudit({
    req,
    action: 'agents.bulkStatus',
    targetType: 'agent',
    targetId: uniqueIds.join(','),
    summary: `${result.count} agent account(s) set to ${body.status}`,
    before,
    after: { agentIds: uniqueIds, status: body.status, updated: result.count },
  });
...
router.get('/agents/:id', async (req, res) => {
  ...
  const [health] = await buildAgentHealthProfiles({ agentIds: [id], includeArchived: true });

  return res.json({ agent, health: health || null, latest: { deals: latestDeals, listings: latestListings, tasks: latestTasks, envelopes: latestEnvelopes } });
});

router.get('/agents/:id/health', async (req, res) => {
  const [health] = await buildAgentHealthProfiles({ agentIds: [req.params.id], includeArchived: true });
  if (!health) return res.status(404).json({ error: 'Not found' });
  return res.json({ health });
});

router.post('/agents/:id/status', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'staff', 'Only owner/admin can update agent status'))) return;
  ...
  await writeInternalAudit({
    ...
    targetType: 'agent',
    targetId: id,
    summary: `${agent.email} status set to ${status}`,
    before,
    after: agent,
  });
...
router.post('/agents/:id/subscription', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'billing', 'Only billing/admin staff can update subscription controls'))) return;
  ...
  const before = await prisma.agent.findUnique({
    where: { id },
    select: { id: true, email: true, name: true, subscriptionStatus: true, billingAccessOverride: true },
  });
  ...
  await writeInternalAudit({
    ...
    targetType: 'agent',
    targetId: id,
    summary: `${agent.email} subscription controls updated`,
    before,
    after: agent,
  });
...
router.put('/legal/policies', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'staff', 'Only owner/admin can update legal policies'))) return;
  ...
  try {
    ...
    const before = await getLegalPolicies();
    ...
    await writeInternalAudit({
      req,
      action: 'legal.policies',
      targetType: 'legalPolicy',
      summary: 'Legal policy URLs/versions updated',
      before,
      after: updated,
    });
...
router.get('/usage', async (req, res) => {
  const hours = Math.min(Math.max(Number(req.query.hours || 720), 1), 87600);
  const agentId = z.string().trim().optional().parse(req.query.agentId);
  const since = new Date(Date.now() - hours * 60 * 60 * 1000);

  const eventScope: Prisma.InternalEventWhereInput = agentId
    ? { agentId }
    : { OR: [{ agentId: null }, { agent: { status: { not: 'REVOKED' } } }] };

  const errorScope: Prisma.InternalErrorWhereInput = agentId
    ? { agentId }
    : { OR: [{ agentId: null }, { agent: { status: { not: 'REVOKED' } } }] };
  ...
  // For large ranges, step by week or month to keep trend chart readable
  ...
  for (let i = totalDays - 1; i >= 0; i -= step) {
    ...
    date.setDate(date.getDate() - i);
    const key = date.toISOString().slice(0, 10);
    // When stepping, aggregate the days in this bucket
    let count = 0;
    for (let d = 0; d < step && (i - d) >= 0; d++) {
      const bDate = new Date();
      bDate.setHours(0, 0, 0, 0);
      bDate.setDate(bDate.getDate() - (i - d));
      count += trendMap.get(bDate.toISOString().slice(0, 10)) || 0;
    }
...
router.get('/system', async (_req, res) => {
  if (!(await requireInternalCapability(_req as AuthenticatedRequest, res, 'system', 'Only owner/admin/engineering can view system diagnostics'))) return;
  ...
  const envChecks = {
    ...
    // Billing in this app runs via server-created Stripe sessions, so a publishable
    // key is optional. Report healthy if either publishable key exists or secret is set.
    stripePublic: Boolean(stripePublishableKey || process.env.STRIPE_SECRET_KEY),
...
router.get('/billing/overview', async (_req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(_req, res, 'billing', 'Only billing/admin staff can view billing overview'))) return;
...
router.get('/billing/agent/:id/subscription', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'billing', 'Only billing/admin staff can view subscription details'))) return;
...
router.get('/billing/agent/:id/payment-method', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'billing', 'Only billing/admin staff can view payment methods'))) return;
...
router.get('/billing/agent/:id/invoices', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'billing', 'Only billing/admin staff can view invoices'))) return;
...
router.post('/billing/agent/:id/settings', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'billing', 'Only billing/admin staff can update billing settings'))) return;
  ...
  const before = await prisma.agent.findUnique({
    where: { id: agentId },
    select: { id: true, email: true, name: true, billingMode: true, billingCustomPriceCents: true, subscriptionStatus: true, billingAccessOverride: true, stripeCustomerId: true },
  });
  ...
  const updated = await prisma.agent.update({
    ...
    data: {
      ...
      // Free plan implies access even if Stripe isn't linked yet or an old trial status lingers.
      ...(body.billingMode === 'FREE' ? { subscriptionStatus: 'ACTIVE', billingAccessOverride: true } : {}),
    },
    select: { id: true, email: true, name: true, billingMode: true, billingCustomPriceCents: true, subscriptionStatus: true, billingAccessOverride: true, stripeCustomerId: true },
  });
  ...
  await writeInternalAudit({
    ...
    targetType: 'agent',
    targetId: agentId,
    summary: `${updated.email} billing mode set to ${updated.billingMode}`,
    before,
    after: updated,
  });
...
router.get('/support', async (req, res) => {
  ...
  const [total, requests, counts, overdueSupport, urgentSupport, unassignedSupport] = await Promise.all([
    ...
    prisma.supportRequest.findMany({
      ...
      include: {
        ...
        assignedToAgent: { select: { id: true, name: true, email: true } },
      },
    }),
    ...
    prisma.supportRequest.count({ where: { status: { in: ['OPEN', 'IN_PROGRESS'] }, dueAt: { lt: new Date() } } }),
    prisma.supportRequest.count({ where: { status: { in: ['OPEN', 'IN_PROGRESS'] }, priority: { in: ['HIGH', 'URGENT'] } } }),
    prisma.supportRequest.count({ where: { status: { in: ['OPEN', 'IN_PROGRESS'] }, assignedToAgentId: null } }),
  ]);
  ...
  return res.json({
    ...
    sla: {
      overdue: overdueSupport,
      urgent: urgentSupport,
      unassigned: unassignedSupport,
    },
  });
});

router.patch('/support/:id', async (req: AuthenticatedRequest, res) => {
  if (!(await requireInternalCapability(req, res, 'support', 'Only support/admin staff can update support requests'))) return;
  ...
  const { reason, assignedToAgentId, ...updates } = parsed.data;
  ...
  if (assignedToAgentId) {
    const assignee = await prisma.internalStaff.findUnique({
      where: { agentId: assignedToAgentId },
      select: { active: true, role: true },
    });
    if (!assignee?.active || !['OWNER', 'ADMIN', 'SUPPORT', 'PRODUCT', 'ENGINEERING'].includes(String(assignee.role))) {
      return res.status(400).json({ error: 'Support requests can only be assigned to active support-capable internal staff' });
    }
  }
  ...
  const data: Prisma.SupportRequestUpdateInput = {
    ...
    ...(updates.priority ? { dueAt: supportDueAtForPriority(updates.priority, before.createdAt) } : {}),
    ...(updates.status === 'IN_PROGRESS' && !before.firstResponseAt ? { firstResponseAt: new Date() } : {}),
    ...(assignedToAgentId !== undefined
      ? { assignedToAgent: assignedToAgentId ? { connect: { id: assignedToAgentId } } : { disconnect: true } }
      : {}),
  };
  ...
  await writeInternalAudit({
    ...
    targetId: updated.id,
    summary: `${updated.subject} updated (${updated.status}/${updated.priority})`,
    before,
    after: updated,
    reason,
  });
