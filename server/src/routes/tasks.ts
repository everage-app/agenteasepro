...
function joinDisplayParts(...values: unknown[]) {
  return values.map(cleanDisplayText).filter(Boolean).join(', ') || undefined;
}

function formatClientName(firstName?: string | null, lastName?: string | null) {
  return joinDisplayParts([firstName, lastName].map(cleanDisplayText).filter(Boolean).join(' '));
}
...
router.get('/', async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const enrichedTasks = tasks.map(task => ({
      ...
      deal: task.deal ? {
        id: task.deal.id,
        title: cleanDisplayText(task.deal.title) || joinDisplayParts(task.deal.property?.street, task.deal.property?.city, task.deal.property?.state) || 'Deal',
        stage: task.deal.status,
        address: joinDisplayParts(task.deal.property?.street, task.deal.property?.city, task.deal.property?.state),
      } : undefined,
      client: task.client ? {
        id: task.client.id,
        name: formatClientName(task.client.firstName, task.client.lastName) || 'Client',
      } : undefined,
      listing: task.listing ? {
        id: task.listing.id,
        address: cleanDisplayText(task.listing.headline),
        mlsId: cleanDisplayText(task.listing.mlsId),
      } : undefined,
      marketingBlast: task.marketingBlast ? {
        id: task.marketingBlast.id,
        title: cleanDisplayText(task.marketingBlast.title) || 'Marketing blast',
      } : undefined,
    }));
