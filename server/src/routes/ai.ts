...
/**
 * POST /api/ai/landing-pages/generate-content
 * Generate headline/subheadline/features for a landing page
 */
router.post('/landing-pages/generate-content', async (req: AuthenticatedRequest, res) => {
  if (!req.agentId) return res.status(401).json({ error: 'Unauthorized' });
  try {
    const { address, beds, baths, sqft, price, propertyType, notes } = req.body as {
      address?: string; beds?: number; baths?: number; sqft?: number; price?: number; propertyType?: string; notes?: string;
    };
    const copy = await draftListingCopy({ address, beds, baths, sqft, price, propertyType, notes });
    const headlineSource = copy.short || address || 'Stunning Property';
    const headline = headlineSource.split('.').slice(0, 1).join('').slice(0, 90) || 'Your next home awaits';
    res.json({
      headline,
      subheadline: copy.short || `Discover everything this ${propertyType || 'home'} has to offer.`,
      description: copy.long,
      features: copy.highlights || [],
    });
  } catch (error) {
    console.error('Landing page content generation error:', error);
