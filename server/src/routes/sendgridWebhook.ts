...
async function createMarketingClickFollowUpTask(params: {
  agentId: string;
  blastId: string;
  email: string;
  ...
  if (!email) return;
  const tag = `marketing-engagement:${params.blastId}:${email}:CLICK`;

  const [client, lead, existing] = await Promise.all([
    prisma.client.findFirst({
      where: { agentId: params.agentId, email: { equals: email, mode: 'insensitive' } },
      select: { id: true, firstName: true, lastName: true },
    }),
    prisma.lead.findFirst({
      where: { agentId: params.agentId, email: { equals: email, mode: 'insensitive' }, converted: false },
      select: { id: true, firstName: true, lastName: true },
    }),
  ...
  await prisma.task.create({
    data: {
      ...
      clientId: client?.id,
      marketingBlastId: params.blastId,
      title: `Follow up with ${contactName || email} - clicked campaign`,
      description: `${tag}\nClicked a marketing email link.${params.url ? `\nURL: ${params.url}` : ''}`,
      category: 'MARKETING',
      priority: 'HIGH',
      bucket: 'TODAY',
      dueAt: params.occurredAt,
      createdFrom: 'SYSTEM',
    },
  });
}
...
export async function sendgridEventsWebhookHandler(req: Request, res: Response) {
  ...
  const clickTaskSeeds: Array<{ agentId: string; blastId: string; email: string; url: string | undefined; occurredAt: Date }> = [];
  ...
  for (const e of events.slice(0, 5000)) {
    ...
    if (agentId && blastId && email && eventType === 'CLICK') {
      clickTaskSeeds.push({ agentId, blastId, email: email.toLowerCase(), url: url || undefined, occurredAt });
    }
  ...
  try {
    ...
    if (clickTaskSeeds.length) {
      const uniqueSeeds = new Map<string, (typeof clickTaskSeeds)[number]>();
      for (const seed of clickTaskSeeds) {
        const key = `${seed.agentId}:${seed.blastId}:${seed.email}`;
        if (!uniqueSeeds.has(key)) uniqueSeeds.set(key, seed);
      }
      await Promise.allSettled(Array.from(uniqueSeeds.values()).slice(0, 50).map((seed) => createMarketingClickFollowUpTask(seed)));
    }
  } catch (err) {
