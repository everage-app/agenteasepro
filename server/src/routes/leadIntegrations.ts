...
import { buildLeadOpsTags, getLeadOpsSettings } from '../services/leadOpsSettings';
...
router.post('/website-leads', async (req, res) => {
  try {
    ...
    const leadOps = await getLeadOpsSettings(agentId);
    ...
    const taggedWithScore = Array.from(new Set([
      ...finalTags,
      ...buildLeadOpsTags(leadOps),
      ...(aiScoreTag ? [aiScoreTag] : []),
    ])).slice(0, 12);
    ...
    const assignedTo = leadOps.assignedTo;
    ...
    const followUpEnabled = leadOps.followUpEnabled;
    if (sequence.length > 0 && !spamTag) {
      const tasks = sequence
        .filter((s: any) => s && s.type && s.minutes)
        .map((step: any) => {
          const minutes = Math.max(5, Number(step.minutes || leadOps.followUpMinutes));
    ...
    } else if (followUpEnabled && !spamTag) {
      const minutes = leadOps.followUpMinutes;
      const dueAt = new Date(Date.now() + minutes * 60 * 1000);
      const hoursAhead = (dueAt.getTime() - Date.now()) / (1000 * 60 * 60);
      const bucket = hoursAhead <= 24 ? 'TODAY' : hoursAhead <= 72 ? 'THIS_WEEK' : 'LATER';
      ...
      await prisma.task.create({
        data: {
          ...
          description: sourceLabel
            ? `New lead from ${sourceLabel}. First response SLA: ${minutes} minutes.`
            : `New lead captured from website. First response SLA: ${minutes} minutes.`,
          category: 'CALL',
          priority: 'NORMAL',
          bucket,
          dueAt,
    ...
    await runLeadCaptureWorkflow({
      ...
      contextPath: '/api/integrations/lead-capture',
      listingId: data.listingId,
      landingPageId: data.landingPageId,
      createTask: false,
    });
