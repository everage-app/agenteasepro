...
const mapMlsListingToResult = (localListing: any): PropertyResult => ({
  source: 'utahrealestate',
  mlsId: localListing.mlsNumber,
  taxId: localListing.raw?.taxId || undefined,
  address: {
  ...
  beds: localListing.beds ? Number(localListing.beds) : undefined,
  baths: localListing.baths ? Number(localListing.baths) : undefined,
  sqft: localListing.squareFeet || undefined,
  lotSize: localListing.lotSize || undefined,
  yearBuilt: localListing.yearBuilt || undefined,
  propertyType: localListing.raw?.propertyType || 'Residential',
  status: localListing.raw?.status || 'Active',
  description: localListing.description || localListing.headline || undefined,
  ...
  daysOnMarket: localListing.raw?.daysOnMarket,
  listingAgent: localListing.raw?.listingAgent,
});
  ...
  try {
    ...
    if (mlsNumber) {
      ...
      if (localListing) {
        ...
        results.push(mapMlsListingToResult(localListing));
      } else {
        // Not in cache - try the dedicated MLS importer first, then broader lookup fallback.
        console.log(`[Search] Fetching fresh data from UtahRealEstate.com for MLS# ${mlsNumber}`);
        try {
          const importedListing = await importMlsListing({ agentId: req.agentId, rawMlsNumber: mlsNumber });
          if (importedListing) {
            console.log(`[Search] ✓ Imported MLS# ${mlsNumber} via MLS importer`);
            results.push(mapMlsListingToResult(importedListing));
          }
        } catch (err) {
          console.log(`[Search] MLS importer failed for MLS# ${mlsNumber}:`, err);
        }
