...
router.get('/overview', async (req, res) => {
  try {
    ...
    const totalLeads = await sq(() => prisma.lead.count({ where: activeLeadWhere }), 0);

    const newLeadsThisWeek = await sq(() => prisma.lead.count({
      where: { ...activeLeadWhere, createdAt: { gte: new Date(Date.now() - 7 * 86400000) } },
    }), 0);

    const totalClientsAllTime = await sq(() => prisma.client.count({ where: { agentId } }), 0);
    ...
    //  LEAD SOURCES
    // ================================================================
    const allLeads = await sq(() => prisma.lead.findMany({
      where: activeLeadWhere,
      select: { source: true, id: true, clientId: true },
    }), []);

    const leadsBySource: Record<string, { leadIds: string[]; clientIds: string[] }> = {};
    for (const lead of allLeads) {
      const src = String(lead.source || 'Unknown').replace(/_/g, ' ');
      const bucket = (leadsBySource[src] ||= { leadIds: [], clientIds: [] });
      bucket.leadIds.push(lead.id);
      if (lead.clientId) bucket.clientIds.push(lead.clientId);
    }
    ...
    const topSources = Object.entries(leadsBySource)
      .map(([source, bucket]) => {
        const closed = bucket.clientIds.filter(id => closedClientIds.has(id)).length;
        return {
          source, leads: bucket.leadIds.length, closed,
          conversionRate: bucket.leadIds.length > 0 ? Math.round((closed / bucket.leadIds.length) * 1000) / 10 : 0,
    ...
    const prevLeadCount = await sq(() => prisma.lead.count({
      where: { ...activeLeadWhere, createdAt: { gte: prevStart, lt: startDate } },
    }), 0);
    const currentLeadCount = await sq(() => prisma.lead.count({
      where: { ...activeLeadWhere, createdAt: { gte: startDate } },
    }), 0);
