...
test.describe('REPC Features', () => {
  ...
  test('should display REPC form steps', async ({ page }) => {
    ...
    // Contracts hub uses responsive layouts; verify key anchors instead of a single brittle text node.
    const contractsPageAnchors = [
      page.getByRole('heading', { name: /Contracts & E-Sign|Contracts/i }).first(),
      page.getByText(/Contracts Workflow|Active Contracts|Templates & PDFs/i).first(),
      page.getByRole('button', { name: /Start a REPC|Open Contracts|New Deal/i }).first(),
    ];

    let hasVisibleAnchor = false;
    for (const anchor of contractsPageAnchors) {
    ...
    const onContractsRoute = /\/contracts(?:\/|$)/i.test(page.url());
    expect(hasVisibleAnchor || onContractsRoute).toBeTruthy();
  });
  ...
  test('should send REPC for e-signature', async ({ page }) => {
    ...
    if (await eSignButton.isVisible()) {
      ...
      const eSignModal = page.locator('[class*="modal"]').filter({ hasText: /signature|sign/i }).first();
      if (await eSignModal.isVisible().catch(() => false)) {
        await expect(eSignModal).toBeVisible();
      } else {
        const signatureText = page.getByText(/signature|sign/i).first();
        if (await signatureText.isVisible().catch(() => false)) {
