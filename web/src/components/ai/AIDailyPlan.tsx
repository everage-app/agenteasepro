...
import { AlertTriangle, CalendarDays, Clock3, Lightbulb } from 'lucide-react';
...
export function AIDailyPlan() {
  ...
  const getUrgencyBadge = (urgency: string) => {
    switch (urgency) {
      ...
      case 'today':
        return (
          <span className="inline-flex items-center rounded-full bg-[#d6b56d]/[0.14] px-2 py-0.5 text-[10px] font-semibold text-[#f2d894]">
            <CalendarDays className="mr-1 h-3 w-3" /> Today
          </span>
        );
      case 'soon':
        return (
          <span className="inline-flex items-center rounded-full bg-slate-400/[0.14] px-2 py-0.5 text-[10px] font-semibold text-slate-200">
            <Clock3 className="mr-1 h-3 w-3" /> Soon
          </span>
        );
    }
  };
  ...
  return (
    <div className="ae-ai-panel rounded-2xl border border-[#f2d894]/[0.22] bg-gradient-to-br from-[#141d2b]/[0.94] via-[#0b1220]/[0.90] to-[#07090d]/[0.94] backdrop-blur-xl p-4 shadow-[0_16px_35px_rgba(0,0,0,0.55)]">
      ...
      {plan && isExpanded && (
        <div className="space-y-4">
          <div className="rounded-2xl border border-[#f2d894]/[0.12] bg-[#0d141f]/[0.70] p-4">
          ...
          <div className="space-y-2">
            {plan.actions.slice(0, 8).map((action) => (
              <div
                ...
                className={`rounded-2xl border border-[#f2d894]/[0.12] bg-[#0d141f]/[0.70] p-4 transition-all cursor-pointer hover:bg-[#141d2b]/[0.86] hover:scale-[1.01] ${
                  action.relatedTo ? 'hover:border-[#f2d894]/[0.45]' : ''
                }`}
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br ${getUrgencyColor(
                      action.urgency
                    )} ${action.urgency === 'today' ? 'text-[#171106]' : 'text-white'} shadow-lg text-xs font-bold`}
                  >
                  ...
                  {action.relatedTo && (
                    <svg className="w-4 h-4 text-[#f2d894] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="mt-4 pt-4 border-t border-white/10">
        <p className="text-[10px] text-slate-500 text-center">
          <Lightbulb className="mr-1 inline h-3 w-3" /> Powered by AI • Focused on Utah real estate
        </p>
      </div>
    </div>
  );
