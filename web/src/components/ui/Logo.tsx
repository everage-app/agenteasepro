...
export function Logo({ size = 'md', showText = true, className = '' }: LogoProps) {
  ...
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      ...
      {/* Text */}
      {showText && (
        <span className={`font-bold tracking-normal ae-logo-text ${s.text}`}>
          <span className="ae-logo-agent">Agent</span><span className="ae-logo-ease font-medium">Ease</span><span className="ae-logo-badge ml-1 rounded-md px-1.5 py-0.5 text-[0.62em] font-bold tracking-normal">Pro</span>
        </span>
      )}
    </div>
  );
