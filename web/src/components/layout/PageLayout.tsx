...
export function PageLayout({
  ...
  return (
    <div className="ae-page-layout min-h-screen bg-transparent pb-20 lg:pb-0">
      {/* Page Header - Stunning gradient in light mode */}
      <div className="ae-page-header border-b border-slate-200/70 bg-white/75 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/30">
            ...
            <nav className="hidden sm:flex mb-4 items-center gap-2 text-sm">
              {breadcrumbs.map((crumb, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  {crumb.href ? (
                    <Link to={crumb.href} className="text-slate-500 hover:text-slate-900 transition-colors dark:text-slate-400 dark:hover:text-white">
                      {crumb.label}
                    </Link>
                  ) : (
      ...
      {/* Page Content */}
      <div className={`ae-page-content mx-auto px-4 sm:px-6 py-4 sm:py-8 ${maxWidthClasses[maxWidth]}`}>
        {children}
      </div>
    </div>
  );
