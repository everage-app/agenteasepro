...
export const leadsApi = {
  getRoutingSlaSettings: () =>
    api.get<{
      routingMode: 'MANUAL' | 'ASSIGN_TO_SELF' | 'LABEL';
      assignedTo?: string;
    ...
    }>('/leads/settings/routing-sla'),

  saveRoutingSlaSettings: (data: {
    routingMode: 'MANUAL' | 'ASSIGN_TO_SELF' | 'LABEL';
    assignToSelf?: boolean;
    assignToLabel?: string;
    followUpEnabled: boolean;
    ...
    warningMinutes: number;
    escalationAction: 'TASK_ONLY' | 'NOTIFY_AGENT';
  }) => api.put('/leads/settings/routing-sla', data),
  ...
  convertLead: (
    ...
    api.post<{ lead: Lead; client: { id: string; firstName: string; lastName: string } }>(`/leads/${id}/convert`, data),
