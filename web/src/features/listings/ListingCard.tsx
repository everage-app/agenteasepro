...
import { Ban, CheckCircle2, Circle, Clipboard, FileText, Home, Hourglass, Lightbulb, MapPin, PauseCircle, Pencil, RefreshCw, Smartphone, Star, X, XCircle } from 'lucide-react';
...
const statusStyles: Record<string, { bg: string; text: string; border: string; icon: LucideIcon }> = {
  DRAFT: {
    bg: 'bg-slate-500/20',
    text: 'text-slate-300',
...
export function ListingCard({ listing, onEdit, onLaunchBlast }: Props) {
  ...
  const listingForceCaptureUrl = `${window.location.origin}/api/integrations/lead-capture?listingId=${listing.id}&label=${encodeURIComponent(addressLine1)}&force=1`;
  ...
  return (
    <article className="ae-theme-card-strong group flex flex-col overflow-hidden rounded-[28px] border border-white/10 bg-slate-950/85 text-xs text-slate-200 shadow-[0_22px_60px_rgba(0,0,0,0.9)] backdrop-blur-xl">
      {/* Image */}
        ...
        <div className="absolute left-3 top-3 flex items-center gap-2">
          {(() => {
            ...
            return (
              ...
              >
                <StatusIcon className={`h-3 w-3 ${status === 'ACTIVE' ? 'fill-current' : ''}`} />
                {statusLabelMap[status] ?? status}
              </span>
            );
          })()}
          {isFeatured && (
            <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-400 to-yellow-300 px-2.5 py-1 text-[10px] font-bold text-slate-900 shadow-lg shadow-amber-400/30">
        ...
        {fullAddress && (
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="ae-theme-button-muted inline-flex items-center gap-1 rounded-full border border-slate-400/30 px-2.5 py-1 text-[10px] font-semibold text-slate-200"
            >
              <Clipboard className="h-3 w-3" /> Copy address
            </button>
          </div>
        )}
      ...
      <div className="flex items-center justify-between gap-2 border-t border-white/10 px-4 py-3 text-[11px]">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={onEdit}
            className="ae-theme-button-muted rounded-full border border-white/15 px-3 py-1 text-slate-100 transition-colors hover:border-cyan-400 hover:text-cyan-100"
          >
            Edit
          </button>
          <button
            ...
            className="ae-theme-button-muted rounded-full border border-white/15 px-2 py-1 text-slate-100 transition-colors hover:border-purple-400 hover:text-purple-100"
      ...
      {showQrModal && (
        ...
        >
          ...
          >
            ...
            {/* Header */}
            <div className="mb-4">
              <h3 className="flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-white"><Smartphone className="h-5 w-5" /> Listing QR Codes</h3>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              {/* Force Capture QR */}
              <div className="rounded-xl border border-rose-300/60 bg-gradient-to-br from-rose-50 to-amber-50 p-4 dark:border-rose-400/30 dark:from-rose-500/10 dark:to-amber-500/10">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-xs font-medium text-slate-700 dark:text-slate-300">Force Capture</div>
                  <span className="rounded-full bg-rose-500/15 px-2 py-0.5 text-[9px] font-bold text-rose-600 dark:text-rose-300">GATED</span>
                </div>
                <div className="flex justify-center mb-3">
                  <div className="rounded-lg bg-white p-2">
                    <QRCodeSVG
                      value={listingForceCaptureUrl}
                      size={100}
                      level="M"
                      includeMargin={false}
                    />
                ...
                <p className="text-[10px] text-slate-500 mb-2 text-center">
                  Every scan = qualified lead
                </p>
                <button
                  onClick={() => copyUrl(listingForceCaptureUrl, 'Force capture URL')}
                  className="w-full rounded-lg border border-rose-300/60 bg-white px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-50 dark:border-rose-400/30 dark:bg-white/5 dark:text-rose-200 dark:hover:bg-white/10"
                >
                  Copy URL
                </button>
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-200 text-[10px] text-slate-500 dark:border-white/10">
              <Lightbulb className="mr-1 inline h-3 w-3" /> Print these QR codes on yard signs, flyers, open house materials, or share digitally to drive traffic.
            </div>
          </div>
        </div>
      )}    </article>
