...
import { AlertTriangle, CalendarDays, Check, ClipboardCheck, Pin, Target, X } from 'lucide-react';
...
export function NewTaskModal({
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        const [dealsRes, clientsRes, leadsRes, listingsRes, blastsRes] = await Promise.all([
          api.get('/deals'),
          api.get('/clients'),
  ...
  useEffect(() => {
    if (!leadId || isClientLocked) return;
    const selected = leads.find((lead) => lead.id === leadId);
    const linkedClientId = selected?.clientId || selected?.client?.id || '';
    if (linkedClientId && !clientId) {
      setClientId(linkedClientId);
    }
  }, [leadId, leads, clientId, isClientLocked]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If agent selected a lead that is not converted yet, convert seamlessly and attach task.
  ...
  const bucketOptions: Array<{ value: 'TODAY' | 'THIS_WEEK' | 'LATER'; label: string; icon: LucideIcon; color: string }> = [
    { value: 'TODAY', label: 'Today', icon: Target, color: 'from-red-500 to-orange-500' },
    { value: 'THIS_WEEK', label: 'This Week', icon: CalendarDays, color: 'from-blue-500 to-cyan-500' },
    { value: 'LATER', label: 'Later', icon: Pin, color: 'from-slate-500 to-slate-400' },
  ];
              ...
              <div className="px-6 pb-4 space-y-4 max-h-[60vh] overflow-y-auto">
                    ...
                    {bucketOptions.map((opt) => {
                      ...
                      >
                        <div className="flex flex-col items-center gap-1">
                          <BucketIcon className={`h-4 w-4 ${bucket === opt.value ? 'text-blue-600 dark:text-blue-300' : 'text-slate-500 dark:text-slate-400'}`} />
                ...
                <div className="pt-2 border-t border-slate-200/80 dark:border-white/12">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {/* Lead */}
                    <div>
                      <label className="block text-xs text-slate-500 dark:text-slate-300 mb-1.5">Lead</label>
                      <select
                        value={leadId}
                        onChange={(e) => {
                      ...
                      {leadId && !((leads.find((lead) => lead.id === leadId)?.clientId) || (leads.find((lead) => lead.id === leadId)?.client?.id)) && (
                        <p className="mt-1 text-[11px] text-amber-600 dark:text-amber-300">Lead will be converted to a client automatically when task is created.</p>
                      )}
                    </div>
