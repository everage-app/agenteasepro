    ...
    >
      ...
      {activeTab === 'pipeline' && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-800/90 to-slate-900/95 border border-slate-700/50 shadow-xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                <StatCard label="Buyer Deals" value={stats.deals.buyerDeals || 0} icon={Home} color="cyan" />
                <StatCard label="Seller Deals" value={stats.deals.sellerDeals || 0} icon={KeyRound} color="purple" />
                <StatCard label="Avg Days to Close" value={stats.deals.avgDaysToClose || '—'} icon={Clock3} color="amber" />
                <StatCard label="Pending Volume" value={formatCurrency(stats.deals.pendingVolume || 0)} icon={BadgeDollarSign} color="emerald" />
              </div>
            </div>
          </div>
          ...
          {stats.clientHealth && (
            <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-800/90 to-slate-900/95 border border-slate-700/50 shadow-xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <StatCard label="Hot Leads" value={stats.clientHealth.hotLeads} icon={Flame} color="red" small />
                ...
                <StatCard label="Overdue" value={stats.clientHealth.overdueFollowUps} icon={AlertTriangle} color="red" small alert={stats.clientHealth.overdueFollowUps > 0} />
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-800/90 to-slate-900/95 border border-slate-700/50 shadow-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard label="Active Listings" value={stats.properties.activeListings} icon={Home} color="cyan" />
              <StatCard label="Pending" value={stats.properties.pendingListings || 0} icon={Hourglass} color="amber" />
              <StatCard label="Sold This Month" value={stats.properties.soldThisMonth} icon={CheckCircle2} color="emerald" />
              <StatCard label="Total Views" value={formatNumber(stats.properties.totalViews)} icon={Eye} color="purple" />
              <StatCard label="Avg List Price" value={formatCurrency(stats.properties.avgListPrice || 0)} icon={DollarSign} color="blue" />
              <StatCard label="Avg Sale Price" value={formatCurrency(stats.properties.avgSalePrice || 0)} icon={BadgeDollarSign} color="emerald" />
              <StatCard label="Avg DOM" value={`${stats.properties.avgDaysOnMarket} days`} icon={CalendarDays} color="slate" />
              <StatCard label="List/Sale Ratio" value={`${stats.properties.listToSaleRatio || 98}%`} icon={BarChart3} color="cyan" />
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-8">
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-800/90 to-slate-900/95 border border-slate-700/50 shadow-xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="flex items-center gap-2 text-lg font-semibold text-white"><PhoneCall className="h-5 w-5 text-cyan-300" /> Call Activity</h3>
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-800/90 to-slate-900/95 border border-slate-700/50 shadow-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard label="Notes Sent" value={stats.calls.totalNotes || 0} icon={Mail} color="purple" />
              <StatCard label="Pop-bys Done" value={stats.calls.totalPopbys || 0} icon={UserPlus} color="emerald" />
              <StatCard label="Referrals Asked" value={stats.calls.totalReferralsAsked || 0} icon={Handshake} color="cyan" />
              <StatCard label="Calls Made" value={stats.calls.totalCalls} icon={PhoneCall} color="blue" />
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'financials' && stats.financials && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard label="Avg Deal Price" value={formatCurrency(stats.financials.avgDealPrice || stats.deals.avgDealValue)} icon={Home} color="blue" />
            <StatCard label="Commission Rate" value={`${stats.financials.commissionRate || 2.5}%`} icon={BarChart3} color="purple" />
            <StatCard label="Deals YTD" value={stats.financials.totalDealsYTD || 0} icon={Handshake} color="cyan" />
            <StatCard label="YTD Volume" value={formatCurrency(stats.financials.totalVolumeYTD || 0)} icon={BadgeDollarSign} color="emerald" />
          </div>
        </div>
      )}
        ...
        <div className="space-y-8">
          {hasTeam && stats.team ? (
            <>
              {/* Team Overview */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard label="Team Members" value={stats.team.memberCount} icon={UsersRound} color="purple" />
                <StatCard label="Active Agents" value={stats.team.activeAgents} icon={CheckCircle2} color="emerald" />
                <StatCard label="Total Clients" value={stats.team.totalClients} icon={Handshake} color="blue" />
                <StatCard label="Team Volume" value={formatCurrency(stats.team.teamVolume || 0)} icon={BadgeDollarSign} color="cyan" />
              </div>
