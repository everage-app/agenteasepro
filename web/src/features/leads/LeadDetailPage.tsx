...
export function LeadDetailPage() {
  ...
  const loadLead = async () => {
    ...
    try {
      ...
      const mailing = resolveLeadMailingAddress(next);
      setDetail(next);
      setDraft({
  ...
  const importedMailingAddress = useMemo(() => resolveLeadMailingAddress(detail), [detail]);
  ...
  const saveLead = async () => {
    ...
    try {
      ...
      const res = await leadsApi.updateLead(id, {
        ...
        mailingCity: draft.mailingCity.trim() || null,
        mailingState: draft.mailingState.trim() || null,
        mailingZip: draft.mailingZip.trim() || null,
        source: draft.source,
  ...
  const openTask = (preset: 'followup' | 'hot') => {
    if (!detail) return;
    const fullName = `${detail.firstName} ${detail.lastName}`.trim();
    if (preset === 'hot') {
      const due = new Date();
      due.setHours(due.getHours() + 2);
      setTaskDefaultTitle(`Hot lead follow-up: ${fullName}`.trim());
      setTaskDefaultCategory('CALL');
      setTaskDefaultDueAt(formatDateTimeLocal(due));
      setShowNewTask(true);
      return;
    }

    setTaskDefaultTitle(`Follow up: ${fullName}`.trim());
    setTaskDefaultCategory('CALL');
  ...
  const convertLead = async (payload?: {
    ...
    try {
      setSaving(true);
      setError(null);
      const res = await leadsApi.convertLead(id, payload);
    ...
    <div className="space-y-7 pb-10">
      <div className="relative overflow-hidden rounded-[34px] border border-indigo-200/70 bg-[radial-gradient(circle_at_top_left,_rgba(96,165,250,0.22),_transparent_32%),radial-gradient(circle_at_bottom_right,_rgba(14,165,233,0.18),_transparent_30%),linear-gradient(135deg,_rgba(255,255,255,0.98),_rgba(241,245,249,0.92))] p-6 shadow-[0_24px_70px_rgba(15,23,42,0.16)] dark:border-white/10 dark:bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.22),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(8,145,178,0.22),_transparent_28%),linear-gradient(135deg,_rgba(3,11,26,0.96),_rgba(6,22,47,0.92))] dark:shadow-[0_30px_90px_rgba(1,8,20,0.72)] sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex max-w-xl flex-wrap items-center justify-start gap-2 xl:justify-end">
            ...
            <Button variant="outline" onClick={() => openTask('followup')}>Add Task</Button>
            <Button variant="outline" onClick={() => openTask('hot')}>Hot Follow-Up</Button>
            ...
            {detail.clientId || detail.client?.id ? (
              ...
              <Button onClick={openConvertModal} disabled={saving}>Convert to Client</Button>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.35fr_0.95fr]">
        <Card title="Agent Playbook" description="What matters right now and the clearest next move for this lead.">
          ...
          <div className="mt-5 rounded-[24px] border border-cyan-300/40 bg-cyan-500/8 p-5 dark:border-cyan-400/20 dark:bg-cyan-400/10">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button size="sm" onClick={() => openTask('followup')}>Create follow-up task</Button>
        ...
        <Card title="Relationship Snapshot" description="A fast read on this contact before you reach out.">
          <div className="space-y-3">
            <SnapshotRow label="Next task" value={detail.nextTask || 'No task committed yet'} onClick={() => openTask('followup')} actionLabel="Create task" />
            <SnapshotRow label="Tags" value={renderTagSummary(detail.tags)} onClick={() => scrollToSection('profile-details')} actionLabel="Edit tags" />
            <SnapshotRow label="Client status" value={detail.clientId || detail.client?.id ? 'Linked client' : detail.converted ? 'Converted lead' : 'Open lead'} onClick={openClientId ? openLinkedClient : openConvertModal} actionLabel={openClientId ? 'Open client' : 'Convert'} />
      ...
      <Card title="Lead Overview" description="The operating facts agents need at a glance.">
        <div className="grid grid-cols-1 gap-2.5 text-xs sm:grid-cols-2 lg:grid-cols-4">
          ...
          <OverviewItem label="Converted" value={detail.clientId || detail.client?.id ? 'Yes (client linked)' : detail.converted ? 'Yes' : 'No'} onClick={openClientId ? openLinkedClient : openConvertModal} actionLabel={openClientId ? 'Open client' : 'Convert'} />
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div id="timeline-notes">
          <UnifiedActivityTimeline 
            leadId={id ?? ""} 
            activities={leadInsights.recentActivities} 
            onActivityCreated={() => void loadLead()} 
          />
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Mailing city">
              <input
                ...
                onChange={(e) => setDraft({ ...draft, mailingCity: e.target.value })}
                className={inputClass}
            ...
            <Field label="State">
              <input
                ...
                onChange={(e) => setDraft({ ...draft, mailingState: e.target.value })}
                className={inputClass}
                placeholder="UT"
              />
            </Field>
            <Field label="ZIP">
              <input
                ...
                onChange={(e) => setDraft({ ...draft, mailingZip: e.target.value })}
                className={inputClass}
                placeholder="84101"
              />
            </Field>
            <div className="sm:col-span-2 -mt-1 text-[11px] text-slate-500 dark:text-slate-400">
              Save mailing info here when the agent already has it so direct-mail exports and client conversion stay one click away.
            </div>
            <div className="sm:col-span-2">
              <Field label="Source">
      ...
      <LeadConvertModal
        open={showConvertModal && !Boolean(openClientId)}
        leadName={`${detail.firstName || ''} ${detail.lastName || ''}`.trim()}
          initialValues={importedMailingAddress}
        saving={saving}
        onClose={() => setShowConvertModal(false)}
        onConfirm={(payload) => void convertLead(payload)}
      />
