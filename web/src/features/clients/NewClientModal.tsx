...
import { clientRankOptions, clientRoleOptions, clientTemperatureOptions } from './clientPresentation';
...
export function NewClientModal({ onClose, onComplete }: NewClientModalProps) {
  ...
  const [formData, setFormData] = useState(NEW_CLIENT_INITIAL);

  const unsaved = useUnsavedChanges({
    draftKey: 'new-client',
    initial: NEW_CLIENT_INITIAL,
    current: formData,
  });

  // Restore draft on mount
  useEffect(() => {
    const draft = unsaved.loadDraft();
    if (draft) {
  ...
  useEscapeKey(handleClose, !unsaved.showConfirmDialog);
                ...
                {draftRestored && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-blue-700 text-sm flex items-center justify-between dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-300">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setFormData(NEW_CLIENT_INITIAL);
                        unsaved.clearDraft();
                      ...
                      return (
                        <button
                          ...
                          className={`group relative rounded-xl border p-3 transition-all duration-200 ${
                            selected
                              ? opt.selectedClass
                              : 'border-slate-200/80 bg-white/80 text-slate-600 hover:border-[#d6b56d]/35 hover:bg-[#f8f3e6]/70 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300 dark:hover:border-[#f2d894]/25 dark:hover:bg-white/[0.07]'
                          }`}
                        >
                          <div className="flex flex-col items-center gap-2">
                            <IconBadge icon={opt.icon} tone={opt.tone} size="md" className={selected ? 'scale-105' : ''} />
                            <span className={`text-xs font-semibold ${selected ? 'text-current' : 'text-slate-600 dark:text-slate-300'}`}>
                              {opt.label}
                            </span>
                          </div>
                        </button>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {clientRankOptions.map((opt) => {
                          const selected = formData.referralRank === opt.value;
                          return (
                        <button
                            key={opt.value}
                            type="button"
                            ...
                            selected
                                ? opt.selectedClass
                                : 'border-slate-200/80 bg-white/75 text-slate-700 hover:border-[#d6b56d]/35 hover:bg-[#f8f3e6]/70 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300 dark:hover:bg-white/[0.08]'
                            }`}
                        >
                            <div className="relative">
                              <IconBadge icon={opt.icon} tone={opt.tone} size="md" shape="circle" />
                              <span className="absolute -bottom-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full border border-white bg-slate-950 text-[9px] font-black text-white shadow-sm dark:border-slate-900">
                                {opt.value}
                              </span>
                            </div>
                            <div className="text-left">
                            <div className={`text-xs font-bold ${selected ? 'text-current' : 'text-slate-700 dark:text-slate-200'}`}>
                                {opt.label}
                            </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {clientTemperatureOptions.map((opt) => {
                          const selected = formData.temperature === opt.value;
                          return (
                        <button
                            key={opt.value}
                            type="button"
