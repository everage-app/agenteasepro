...
import { clientTemperatureMeta } from './clientPresentation';
...
type ClientSegment = 'ALL' | 'NEEDS_ACTION' | 'DEADLINES' | 'NO_CONTACT' | 'HOT' | 'ACTIVE_DEAL' | 'MISSING_ADDRESS';
...
const formatContactAge = (client: ClientListItem) => {
...
const hasActiveDeal = (client: ClientListItem) => Boolean(client.primaryProperty || client.primaryDealStage === 'UNDER_CONTRACT');
const hasCompleteMailingAddress = (client: ClientListItem) =>
  Boolean(client.mailingAddress && client.mailingCity && client.mailingState && client.mailingZip);

const getClientNextMove = (client: ClientListItem) => {
  const lastContactDays = daysSince(client.lastContactAt);
  const deadlineDays = daysUntil(client.nextDeadline);

  if (deadlineDays !== null && deadlineDays <= 3) {
    return {
      label: deadlineDays < 0 ? 'Resolve missed deadline' : deadlineDays === 0 ? 'Handle deadline today' : 'Deadline coming up',
      detail: client.nextDeadline ? new Date(client.nextDeadline).toLocaleDateString() : 'Date needs review',
      tone: 'urgent' as const,
      icon: CalendarClock,
    };
  }

  if (client.openTasksCount > 0) {
    return {
      label: 'Work open tasks',
      detail: `${client.openTasksCount} task${client.openTasksCount === 1 ? '' : 's'} waiting`,
      tone: 'warning' as const,
      icon: CheckCircle2,
    };
  }

  if (client.temperature === 'HOT' && lastContactDays >= 2) {
    return {
      label: 'Call hot client',
      detail: formatContactAge(client),
      tone: 'urgent' as const,
      icon: Phone,
    };
  }

  if (!Number.isFinite(lastContactDays) || lastContactDays >= 14) {
    return {
      label: 'Re-engage relationship',
      detail: formatContactAge(client),
      tone: 'warning' as const,
      icon: Clock3,
    };
  ...
  if (!hasCompleteMailingAddress(client)) {
    return {
      label: 'Capture mailing address',
      detail: 'Needed for direct mail and print campaigns',
      tone: 'warning' as const,
      icon: Mail,
    };
  }

  if (!hasActiveDeal(client) && ['ACTIVE', 'NEW_LEAD', 'NURTURE'].includes(client.stage)) {
    return {
      label: 'Qualify next deal',
      detail: 'No active deal linked',
      tone: 'strong' as const,
      icon: Target,
    };
  }

  if (hasActiveDeal(client)) {
    return {
      label: 'Protect active deal',
      detail: client.primaryProperty || 'Deal in motion',
...
const clientMoveClass = (tone: ReturnType<typeof getClientNextMove>['tone']) => {
...
export function ClientsListPage() {
  ...
  const segmentedCounts = useMemo(() => {
    ...
    const hot = clients.filter((c) => c.temperature === 'HOT').length;
    const activeDeal = clients.filter(hasActiveDeal).length;
    const missingAddress = clients.filter((c) => !hasCompleteMailingAddress(c)).length;
    return { noContact, deadlines, needsAction, hot, activeDeal, missingAddress };
  }, [clients]);
  ...
  const filteredClients = useMemo(() => {
    ...
    if (segmentFilter === 'HOT') list = list.filter((c) => c.temperature === 'HOT');
    if (segmentFilter === 'ACTIVE_DEAL') list = list.filter(hasActiveDeal);
    if (segmentFilter === 'MISSING_ADDRESS') list = list.filter((c) => !hasCompleteMailingAddress(c));
  ...
  const clientCommandCenter = useMemo(() => {
    const ranked = [...clients].sort((a, b) => {
      const moveA = getClientNextMove(a);
      const moveB = getClientNextMove(b);
      const rank = { urgent: 0, warning: 1, strong: 2, success: 3, normal: 4 } as const;
      return rank[moveA.tone] - rank[moveB.tone] || daysSince(b.lastContactAt) - daysSince(a.lastContactAt);
    });

    return {
      firstMove: ranked[0] || null,
      hotClients: clients.filter((c) => c.temperature === 'HOT'),
      noDealReady: clients.filter((c) => !hasActiveDeal(c) && ['ACTIVE', 'NEW_LEAD', 'NURTURE'].includes(c.stage)),
    };
  }, [clients]);

  const selectedClients = useMemo(() => {
    if (selectedClientIds.size === 0) return [] as ClientListItem[];
    return filteredClients.filter((client) => selectedClientIds.has(client.id));
  ...
  const selectedWithEmail = selectedClients.filter((client) => String(client.email || '').trim().length > 0);
  const selectedWithPhone = selectedClients.filter((client) => String(client.phone || '').trim().length > 0);
  const selectedPastClientCount = selectedClients.filter((client) => client.stage === 'PAST_CLIENT').length;

  useEffect(() => {
    setSelectedClientIds((prev) => {
      if (prev.size === 0) return prev;
      const visibleIds = new Set(filteredClients.map((client) => client.id));
  ...
  useEffect(() => {
    ...
    selectAllVisibleRef.current.indeterminate = selectedClientIds.size > 0 && !allVisibleSelected;
  }, [selectedClientIds.size, allVisibleSelected]);
  ...
  const runBulkClientAction = async (
    actionLabel: string,
    worker: (client: ClientListItem) => Promise<unknown>,
    onDone?: () => void,
    targetClients?: ClientListItem[],
  ) => {
    const scopedClients = targetClients || selectedClients;
    if (scopedClients.length === 0 || bulkSaving) return;
    try {
      setBulkSaving(true);
      setBulkError(null);
      const jobs = scopedClients.map((client) => worker(client));
      ...
      if (failed > 0) {
        setBulkError(`${actionLabel}: ${succeeded} succeeded, ${failed} failed.`);
      } else {
        setBulkError(`${actionLabel}: ${succeeded} client${succeeded === 1 ? '' : 's'} updated.`);
      }

      if (onDone) onDone();
    } catch (error) {
      console.error(`Bulk client action failed (${actionLabel}):`, error);
      setBulkError(`${actionLabel} failed.`);
    } finally {
      setBulkSaving(false);
    }
  };
  ...
  const openBulkCampaignEmail = () => {
    if (selectedWithEmail.length === 0) {
      setBulkError('Select at least one client with an email address.');
      return;
    }
    const recipientEmails = selectedWithEmail
      .map((client) => String(client.email || '').trim())
      .filter(Boolean)
    ...
    navigate('/marketing', {
      state: {
        massEmailPrefill: {
          ...
          subject: 'Quick check-in from your AgentEasePro team',
          sourceLabel: 'selected clients',
        },
      },
    });
  };
  ...
  const saveBulkNote = async () => {
    ...
    await runBulkClientAction('Bulk note', (client) => api.post(`/clients/${client.id}/notes`, { text: note }), () => {
  ...
  const bulkUpdateStage = async (stage: string) => {
    await runBulkClientAction(`Stage set to ${stage.replace('_', ' ')}`, (client) => api.put(`/clients/${client.id}`, { stage }));
  };

  const bulkArchiveClients = async () => {
    const targets = selectedClients.filter((client) => client.stage !== 'PAST_CLIENT');
    if (targets.length === 0) {
      setBulkError('All selected clients are already archived.');
      return;
    }
    await runBulkClientAction('Archive selected', (client) => api.put(`/clients/${client.id}`, { stage: 'PAST_CLIENT' }), undefined, targets);
  };

  const bulkDeleteClients = async () => {
    if (selectedClients.length === 0) return;
    const shouldDelete = window.confirm(
      `Delete ${selectedClients.length} selected client${selectedClients.length === 1 ? '' : 's'} permanently? This cannot be undone.`,
    );
    if (!shouldDelete) return;

    await runBulkClientAction('Delete selected', (client) => api.delete(`/clients/${client.id}`), () => {
    ...
    >
      ...
      <Card tone="subtle" className="mt-4 p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-slate-950/60 border border-white/16 backdrop-blur-xl shadow-[0_22px_60px_rgba(0,0,0,0.5)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-[10px] uppercase tracking-[0.18em] text-slate-400">Client Command Center</div>
            <div className="text-sm sm:text-base font-semibold text-white">
              {clientCommandCenter.firstMove ? `Next best move: ${getClientNextMove(clientCommandCenter.firstMove).label}` : 'Keep every client moving forward'}
            </div>
            <div className="text-[11px] text-slate-400 mt-1">
              {relationshipStats.overdueFollowUps} overdue follow-ups · {relationshipStats.noContactLogged} no contact logged · {relationshipStats.upcomingDeadlines} deadlines this week · {clientCommandCenter.noDealReady.length} ready to qualify · {segmentedCounts.missingAddress} missing mailing info
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => setSegmentFilter('HOT')}
              className="rounded-full border border-rose-400/30 bg-rose-500/10 px-3 py-1.5 text-[10px] font-semibold text-rose-200 hover:bg-rose-500/20"
            >
              Show hot
            </button>
            <button
            ...
            <button
              onClick={() => setSegmentFilter('ACTIVE_DEAL')}
            ...
            <button
              onClick={() => setSegmentFilter('MISSING_ADDRESS')}
        ...
        <div className="flex flex-col gap-3 sm:gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() => setSegmentFilter('ACTIVE_DEAL')}
              className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${
                segmentFilter === 'ACTIVE_DEAL'
                  ? 'bg-emerald-500/20 border-emerald-400/40 text-emerald-200'
                  : 'bg-white/5 border-white/10 text-slate-300'
              }`}
            >
              Active deals ({segmentedCounts.activeDeal})
            </button>
            <button
              onClick={() => setSegmentFilter('NO_CONTACT')}
              className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${
            ...
            <button
              onClick={() => setSegmentFilter('MISSING_ADDRESS')}
              className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${
                segmentFilter === 'MISSING_ADDRESS'
                  ? 'bg-violet-500/20 border-violet-400/40 text-violet-200'
                  : 'bg-white/5 border-white/10 text-slate-300'
              }`}
            >
              Missing mailing ({segmentedCounts.missingAddress})
            </button>
          </div>
          {/* Action Buttons Row */}
          ...
          {selectedClientIds.size > 0 && (
            <div className="rounded-2xl border border-cyan-300/30 bg-gradient-to-r from-cyan-500/15 via-blue-500/10 to-slate-900/30 p-3 shadow-[0_16px_45px_rgba(14,116,144,0.2)]">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <div className="text-[11px] uppercase tracking-[0.18em] text-cyan-200/90">Client Command Batch</div>
                  <div className="mt-1 text-sm text-cyan-100">{selectedClientIds.size} selected client{selectedClientIds.size === 1 ? '' : 's'}</div>
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px]">
                    ...
                    <span className="rounded-full border border-cyan-300/25 bg-cyan-500/10 px-2 py-0.5 text-cyan-100">{selectedPastClientCount} archived</span>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg border border-white/20 bg-white/10 text-[11px] font-semibold text-white hover:bg-white/15 disabled:opacity-50"
                  >
                    Email blast
                  </button>
                  <button
                    ...
                    disabled={bulkSaving}
                    className="px-3 py-1.5 rounded-lg border border-cyan-300/30 bg-cyan-500/20 text-[11px] font-semibold text-cyan-100 hover:bg-cyan-500/30 disabled:opacity-50"
                  >
                    Add shared note
                  </button>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (!value) return;
                      void bulkUpdateStage(value);
                      e.currentTarget.value = '';
                    }}
                    disabled={bulkSaving}
                    className="px-3 py-1.5 rounded-lg border border-white/20 bg-slate-900/70 text-[11px] text-slate-100"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => void bulkArchiveClients()}
                    disabled={bulkSaving || selectedPastClientCount === selectedClients.length}
                    className="px-3 py-1.5 rounded-lg border border-amber-300/30 bg-amber-500/15 text-[11px] font-semibold text-amber-100 hover:bg-amber-500/25 disabled:opacity-50"
                  >
                    Archive
                  </button>
                  <button
                    type="button"
                    onClick={() => void bulkDeleteClients()}
                    disabled={bulkSaving}
                    className="px-3 py-1.5 rounded-lg border border-rose-300/30 bg-rose-500/15 text-[11px] font-semibold text-rose-100 hover:bg-rose-500/25 disabled:opacity-50"
                  >
                    Delete
                  </button>
                  <button
                    ...
                    disabled={bulkSaving}
                    className="px-3 py-1.5 rounded-lg border border-white/20 bg-slate-900/60 text-[11px] font-semibold text-slate-100 hover:bg-slate-800 disabled:opacity-50"
                  >
                    Clear
                  </button>
                </div>
              </div>
              {bulkError && <div className="mt-2 text-xs text-cyan-100/90">{bulkError}</div>}
            </div>
          )}
        </div>
      </Card>
      ...
      {loading && !clients.length ? (
        ...
        <>
        <div className="mb-3 flex items-center justify-between rounded-xl border border-white/10 bg-slate-950/40 px-3 py-2">
          <label className="inline-flex items-center gap-2 text-xs text-slate-200">
            ...
            />
            Select all visible ({filteredClients.length})
          </label>
          <div className="text-xs text-slate-400">Tip: select cards, then run a bulk action above.</div>
        </div>
          ...
          {filteredClients.map((client) => (
            <div key={client.id} onClick={() => navigate(`/clients/${client.id}`)}>
              <Card
                tone="solid"
                hover
                className={`p-4 backdrop-blur-xl transition-all duration-300 cursor-pointer group ${
                  selectedClientIds.has(client.id)
                    ? 'bg-cyan-500/10 border-cyan-300/35 shadow-[0_16px_40px_rgba(14,116,144,0.25)]'
                    : 'bg-slate-950/40 border-white/10 hover:bg-slate-900/60 hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)]'
                }`}
              >
              {/* Top Row */}
              <div className="flex justify-between items-start mb-3 gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    ...
                    {client.temperature && clientTemperatureMeta[client.temperature] && (() => {
                      const temperature = clientTemperatureMeta[client.temperature];
                      const TemperatureIcon = temperature.icon;
                      return (
                        <Badge variant="default" className={`${temperature.badgeClass} px-1.5`}>
                          <TemperatureIcon className="mr-1 h-3 w-3" strokeWidth={2.2} />
                          {client.temperature}
                        </Badge>
                      );
                    })()}
              ...
              {(() => {
                const nextMove = getClientNextMove(client);
                const MoveIcon = nextMove.icon;
                return (
                  <div className={`mb-3 rounded-xl border px-3 py-2.5 ${clientMoveClass(nextMove.tone)}`}>
                    <div className="flex items-start gap-2.5">
                      <MoveIcon className="mt-0.5 h-4 w-4 shrink-0" />
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-xs font-bold" title={nextMove.label}>{nextMove.label}</div>
              ...
              <div className="flex gap-4 text-xs text-slate-400 mb-4">
                ...
                {client.phone && (
                  <div className="flex items-center gap-1 truncate" title={client.phone}>
                    <Phone className="h-3.5 w-3.5 shrink-0" /> {formatPhoneDisplay(client.phone)}
                  </div>
                )}
              </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {client.phone && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-400/30 text-emerald-200 text-[10px] font-semibold hover:bg-emerald-500/20"
                    >
                  ...
                  {client.phone && (
                    <a
                      href={phoneToSmsHref(client.phone)}
                      onClick={(e) => e.stopPropagation()}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-500/10 border border-blue-400/30 text-blue-200 text-[10px] font-semibold hover:bg-blue-500/20"
                    >
                  ...
                  {client.email && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-purple-500/10 border border-purple-400/30 text-purple-200 text-[10px] font-semibold hover:bg-purple-500/20"
                    >
                ...
                <div>
                  {(() => {
                    if (!client.lastContactAt) {
                      return <span className="inline-flex items-center gap-1 text-rose-400 font-medium"><AlertTriangle className="h-3 w-3" /> Never contacted</span>;
                    }
                    ...
                    if (days <= 7) {
                      ...
                      return <span className="inline-flex items-center gap-1 text-rose-400 font-medium"><AlertTriangle className="h-3 w-3" /> Last contact: {days}d ago</span>;
                    }
                  })()}
                </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <Card className="w-full max-w-lg p-6 bg-slate-950/95 border border-white/10 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="rounded-lg p-1.5 text-slate-400 hover:bg-white/10 hover:text-white" onClick={() => setShowAddToDeal(false)} aria-label="Close add client to deal">
                  <X className="h-4 w-4" />
                </button>
              </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <Card className="w-full max-w-lg p-6 bg-slate-950/95 border border-white/10 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="rounded-lg p-1.5 text-slate-400 hover:bg-white/10 hover:text-white" onClick={() => setShowMergeClients(false)} aria-label="Close merge client">
                  <X className="h-4 w-4" />
                </button>
              </div>
      ...
      {showBulkNote && selectedClientIds.size > 0 && (
        <div className="fixed inset-0 z-50">
          <div
          ...
          />
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <Card className="w-full max-w-xl p-6 bg-slate-950/95 border border-white/10 shadow-2xl">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-lg font-semibold text-white">Add Shared Note</h3>
                  <p className="mt-1 text-xs text-slate-400">
                    This note will be added to {selectedClientIds.size} selected client{selectedClientIds.size === 1 ? '' : 's'}.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-4">
                <textarea
                  ...
                  rows={5}
                  placeholder="Add one note for every selected client (example: weekly check-in sent, awaiting doc upload)."
                  className="w-full bg-slate-900/60 border border-white/10 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400/40"
                />
              </div>

              {bulkNoteError && <div className="mt-2 text-xs text-rose-300">{bulkNoteError}</div>}
