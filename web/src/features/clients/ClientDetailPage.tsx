...
import { ContactSmsModal } from '../../components/communications/ContactSmsModal';
import { clientRankMeta, clientTemperatureMeta } from './clientPresentation';
...
export function ClientDetailPage() {
  ...
  const [taskDefaultCategory, setTaskDefaultCategory] = useState<'GENERAL' | 'CONTRACT' | 'MARKETING' | 'CALL' | 'NOTE' | 'POPBY' | 'EVENT'>('CALL');
  const [taskDefaultDueAt, setTaskDefaultDueAt] = useState<string | undefined>(undefined);
  const [creatingNurtureSequence, setCreatingNurtureSequence] = useState(false);
  const [showMarketingCampaign, setShowMarketingCampaign] = useState(false);
  const [showContactEmail, setShowContactEmail] = useState(false);
  const [showContactSms, setShowContactSms] = useState(false);
  ...
  const openClientTask = () => {
    ...
    setTaskDefaultCategory('CALL');
    setTaskDefaultDueAt(undefined);
    setShowNewTask(true);
  };
  ...
  const stageSequenceLabelMap: Record<string, string> = {
    NEW_LEAD: 'Lead Warmup Sequence',
    NURTURE: 'Nurture Sequence',
    ACTIVE: 'Active Client Sequence',
    UNDER_CONTRACT: 'Contract Sequence',
    CLOSED: 'Closing Follow-Through',
    PAST_CLIENT: 'Post-Close Referral Sequence',
    DEAD: 'Reactivation Sequence',
  };

  const stageSequenceTasks = (stage: string) => {
    ...
    switch (stage) {
      case 'NEW_LEAD':
        return [
          { title: `Initial discovery call: ${fullName}`, category: 'CALL', priority: 'HIGH', bucket: 'TODAY', days: 0 },
          { title: `Send intro resources to ${firstName}`, category: 'MARKETING', priority: 'NORMAL', bucket: 'THIS_WEEK', days: 2 },
          { title: `Confirm needs + timeline for ${firstName}`, category: 'NOTE', priority: 'NORMAL', bucket: 'THIS_WEEK', days: 4 },
        ];
      case 'ACTIVE':
        return [
          { title: `Status call with ${fullName}`, category: 'CALL', priority: 'HIGH', bucket: 'TODAY', days: 0 },
          { title: `Share listing and market updates with ${firstName}`, category: 'MARKETING', priority: 'NORMAL', bucket: 'THIS_WEEK', days: 3 },
          { title: `Align next milestone for ${firstName}`, category: 'NOTE', priority: 'NORMAL', bucket: 'THIS_WEEK', days: 7 },
        ];
      case 'UNDER_CONTRACT':
        return [
          { title: `Review contingency timeline with ${fullName}`, category: 'CONTRACT', priority: 'HIGH', bucket: 'TODAY', days: 0 },
          { title: `Confirm lender/title progress for ${firstName}`, category: 'CALL', priority: 'HIGH', bucket: 'THIS_WEEK', days: 2 },
          { title: `Prepare pre-close checklist for ${firstName}`, category: 'NOTE', priority: 'NORMAL', bucket: 'THIS_WEEK', days: 5 },
        ];
      case 'CLOSED':
        return [
          { title: `48-hour satisfaction check with ${fullName}`, category: 'CALL', priority: 'NORMAL', bucket: 'TODAY', days: 0 },
          { title: `Send home care + move-in resources to ${firstName}`, category: 'MARKETING', priority: 'NORMAL', bucket: 'THIS_WEEK', days: 5 },
          { title: `Request testimonial from ${firstName}`, category: 'NOTE', priority: 'LOW', bucket: 'LATER', days: 14 },
        ];
      case 'PAST_CLIENT':
        return [
          { title: `Referral check-in call: ${fullName}`, category: 'CALL', priority: 'NORMAL', bucket: 'TODAY', days: 0 },
          { title: `Send neighborhood market snapshot to ${firstName}`, category: 'MARKETING', priority: 'LOW', bucket: 'THIS_WEEK', days: 7 },
          { title: `Invite ${firstName} to client appreciation event`, category: 'EVENT', priority: 'LOW', bucket: 'LATER', days: 21 },
        ];
      case 'DEAD':
        return [
          { title: `Reactivation call attempt: ${fullName}`, category: 'CALL', priority: 'NORMAL', bucket: 'TODAY', days: 0 },
          { title: `Send re-engagement message to ${firstName}`, category: 'MARKETING', priority: 'LOW', bucket: 'THIS_WEEK', days: 4 },
          { title: `Final follow-up note for ${firstName}`, category: 'NOTE', priority: 'LOW', bucket: 'LATER', days: 14 },
        ];
      case 'NURTURE':
      default:
        return [
          { title: `Nurture touchpoint call: ${fullName}`, category: 'CALL', priority: 'NORMAL', bucket: 'TODAY', days: 0 },
          { title: `Send market insight update to ${firstName}`, category: 'MARKETING', priority: 'NORMAL', bucket: 'THIS_WEEK', days: 4 },
          { title: `Plan next nurture check-in with ${firstName}`, category: 'NOTE', priority: 'LOW', bucket: 'LATER', days: 10 },
        ];
    }
  };

  const createNurtureSequence = async () => {
    if (creatingNurtureSequence) return;
    setCreatingNurtureSequence(true);
    try {
      const now = new Date();
      ...
      const sequence = stageSequenceTasks(client.stage);
      await Promise.all(
        sequence.map((item) =>
          api.post('/tasks', {
            title: item.title,
      ...
      console.error('Failed to create nurture sequence:', error);
    } finally {
      setCreatingNurtureSequence(false);
    }
  };
  ...
  const openContactSms = () => {
    if (!client.phone) return;
    setShowContactSms(true);
  };
        ...
        <div className="relative flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div>
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {client.temperature && clientTemperatureMeta[client.temperature] && (() => {
                    ...
                    return (
                      <Badge variant="default" className={`${temperature.badgeClass} px-2`}>
                        <TemperatureIcon className="mr-1.5 h-3.5 w-3.5" strokeWidth={2.2} />
                        {client.temperature}
                      </Badge>
                    );
                  })()}
            ...
            <div className="flex flex-wrap gap-4 text-sm text-slate-300/90 mt-1">
              ...
              {client.phone && (
                ...
                >
                  <Phone className="h-4 w-4" /> {formatPhoneDisplay(client.phone)}
                </a>
              )}
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {client.phone && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 hover:border-cyan-400/40 hover:bg-white/10 transition-colors text-xs font-medium text-slate-200"
                >
                  {copied === 'phone' ? <ClipboardCheck className="h-3.5 w-3.5" /> : <Clipboard className="h-3.5 w-3.5" />}
              ...
              {client.email && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 hover:border-cyan-400/40 hover:bg-white/10 transition-colors text-xs font-medium text-slate-200"
                >
                  {copied === 'email' ? <ClipboardCheck className="h-3.5 w-3.5" /> : <Clipboard className="h-3.5 w-3.5" />}
                ...
                <div className="flex items-start gap-3">
                  ...
                  >
                    <span className="inline-flex items-center gap-1.5">
                      {copied === 'address' ? <ClipboardCheck className="h-3.5 w-3.5" /> : <Clipboard className="h-3.5 w-3.5" />}
          ...
          {client.referralRank && (
            <div className="mt-3">
              ...
              <Badge
                variant="default"
                className={clientRankMeta[client.referralRank]?.badgeClass || 'bg-slate-500/10 text-slate-300 border-slate-400/40'}
              >
                {(() => {
                  const RankIcon = clientRankMeta[client.referralRank]?.icon || Sparkles;
                  return <RankIcon className="mr-1.5 h-3.5 w-3.5" strokeWidth={2.2} />;
                })()}
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-stretch md:items-start">
            ...
            <Button
              variant="secondary"
              className="bg-indigo-600/85 border-indigo-500/40 text-white hover:bg-indigo-500 disabled:opacity-60"
              onClick={createNurtureSequence}
              disabled={creatingNurtureSequence}
            >
              {creatingNurtureSequence ? 'Creating sequence...' : `+ ${stageSequenceLabelMap[client.stage] || 'Smart Sequence'}`}
            </Button>
              ...
              <div className="relative pl-4 sm:pl-6">
                ...
                <div className="space-y-4">
                  {(timeline || []).map((item, index) => {
                    ...
                    const getEventIcon = () => {
                      if (isDeal) return FileText;
                      if (isTask) return CheckCircle2;
                      if (isNote) return Clipboard;
                      if (isMarketing) return Megaphone;
                      return Pin;
                    };
                        ...
                        <div className="flex flex-col items-center">
                          <div className={`mt-1 flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br ${getEventColor()} shadow-lg`}>
                            {(() => {
                              ...
                              return <EventIcon className="h-5 w-5 text-white" />;
                            })()}
                          </div>
                        </div>
                        <Card
