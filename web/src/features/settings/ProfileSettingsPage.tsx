...
const writeProfileCache = (cacheKey: string, data: ProfileCache) => {
  ...
  try {
    ...
    // Ignore cache write failures (quota, private mode, etc.) without blocking profile updates.
  }
};
...
export function ProfileSettingsPage() {
  ...
  const applyProfileState = (values: Partial<ProfileCache>) => {
    ...
    setPhotoUrl(normalizeAssetUrl(values.photoUrl));
  ...
  const buildProfileSnapshot = (data: ProfileData): ProfileCache => {
    ...
    return {
      ...
      photoUrl: normalizeAssetUrl(settings.photoUrl),
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        writeProfileCache(profileCacheKey, {
          ...profileSnapshot,
          photoUrl: toCacheSafeAssetUrl(profileSnapshot.photoUrl),
        });
      } catch (error) {
  ...
  const handleProfilePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setSaveMessage({ type: 'error', text: 'Photo file must be under 5MB' });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const freshUrl = withAssetCacheBust(data.photoUrl) || normalizeAssetUrl(data.photoUrl);
      setPhotoUrl(freshUrl);

      // Update localStorage cache so other components pick it up
      const cacheAgentKey = agent?.id || agent?.email || 'anonymous';
      const profileCacheKey = getProfileCacheKey(cacheAgentKey);
      const cached = readProfileCache(profileCacheKey);
      if (cached) {
        writeProfileCache(profileCacheKey, {
          ...cached,
          photoUrl: toCacheSafeAssetUrl(freshUrl),
        });
      }

      dispatchSettingsAssetsUpdated({ profilePhotoUrl: freshUrl });

      setSaveMessage({ type: 'success', text: 'Photo uploaded successfully!' });
      setTimeout(() => setSaveMessage(null), 3000);

      // Reset file input so uploading the same file again triggers change
      e.target.value = '';
    } catch (error) {
  ...
  const handleRemovePhoto = async () => {
    try {
      ...
      const cached = readProfileCache(profileCacheKey);
      if (cached) {
        writeProfileCache(profileCacheKey, { ...cached, photoUrl: null });
      }

      dispatchSettingsAssetsUpdated({ profilePhotoUrl: null });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      writeProfileCache(profileCacheKey, {
        ...optimisticSnapshot,
        photoUrl: toCacheSafeAssetUrl(optimisticSnapshot.photoUrl),
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {saveMessage && (
        (() => {
          const MessageIcon = saveMessage.type === 'success' ? CheckCircle2 : AlertTriangle;
          return (
