...
import { AlertTriangle, CheckCircle2, Clock3, Mail, Phone, Route, Shuffle } from 'lucide-react';
...
type LeadItem = Lead;
type RoutingMode = 'MANUAL' | 'ASSIGN_TO_SELF' | 'LABEL';

interface RoutingSlaSettings {
...
const defaultRoutingSlaSettings: RoutingSlaSettings = {
  routingMode: 'ASSIGN_TO_SELF',
  assignToSelf: true,
  assignToLabel: '',
  followUpEnabled: true,
  followUpMinutes: 15,
  warningMinutes: 10,
  escalationAction: 'TASK_ONLY',
};
...
export function LeadsSettingsPage() {
  ...
  const [routingSla, setRoutingSla] = useState<RoutingSlaSettings>(defaultRoutingSlaSettings);
  ...
  useEffect(() => {
    loadLeads();
    loadRoutingSlaSettings();
  }, []);

  const loadRoutingSlaSettings = async () => {
    try {
      setRoutingLoading(true);
      const res = await leadsApi.getRoutingSlaSettings();
      setRoutingSla({ ...defaultRoutingSlaSettings, ...res.data });
    } catch (error) {
      console.error('Failed to load routing/SLA settings:', error);
      setRoutingError('Unable to load routing settings.');
    } finally {
      setRoutingLoading(false);
    }
  };

  const saveRoutingSlaSettings = async () => {
    try {
      setRoutingSaving(true);
      setRoutingNotice(null);
      setRoutingError(null);
      await leadsApi.saveRoutingSlaSettings({
        routingMode: routingSla.routingMode,
        assignToSelf: routingSla.routingMode === 'ASSIGN_TO_SELF',
        assignToLabel: routingSla.assignToLabel,
        followUpEnabled: routingSla.followUpEnabled,
        followUpMinutes: Number(routingSla.followUpMinutes) || 15,
        warningMinutes: Number(routingSla.warningMinutes) || 10,
        escalationAction: routingSla.escalationAction,
      });
      setRoutingNotice('Routing and first-response SLA saved.');
      await loadRoutingSlaSettings();
    } catch (error) {
      console.error('Failed to save routing/SLA settings:', error);
      setRoutingError('Unable to save routing settings.');
    } finally {
      ...
      <Card className="p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-white">Lead Routing + SLA</h2>
            </div>
            <p className="mt-1 text-sm text-slate-400">New leads get assignment, SLA tags, and first-response tasks.</p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs">
            <span className="inline-flex items-center gap-1 rounded-full border border-slate-400/30 bg-white/5 px-3 py-1 text-slate-200">
              <CheckCircle2 className="h-3.5 w-3.5 text-emerald-300" /> {routingSla.readiness?.openLeadCount ?? 0} open
            </span>
            <span className="inline-flex items-center gap-1 rounded-full border border-amber-400/30 bg-amber-500/10 px-3 py-1 text-amber-200">
              <AlertTriangle className="h-3.5 w-3.5" /> {routingSla.readiness?.unassignedLeadCount ?? 0} unassigned
            </span>
            <span className="inline-flex items-center gap-1 rounded-full border border-rose-400/30 bg-rose-500/10 px-3 py-1 text-rose-200">
              <Clock3 className="h-3.5 w-3.5" /> {routingSla.readiness?.overdueFirstResponseCount ?? 0} overdue
            </span>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Assignment</div>
            <div className="grid gap-2 sm:grid-cols-3">
              {([
                ...
                <button
                  key={mode}
                  type="button"
                  onClick={() => setRoutingSla((prev) => ({ ...prev, routingMode: mode, assignToSelf: mode === 'ASSIGN_TO_SELF' }))}
                  className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
                    routingSla.routingMode === mode
            ...
            {routingSla.routingMode === 'LABEL' && (
              <input
                value={routingSla.assignToLabel || ''}
                onChange={(e) => setRoutingSla((prev) => ({ ...prev, assignToLabel: e.target.value }))}
                placeholder="Team, ISA, or agent label"
                className="mt-3 w-full rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/40"
              />
            )}
          </div>
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="text-xs font-semibold uppercase tracking-wider text-slate-400">First Response</div>
              <button
                type="button"
                onClick={() => setRoutingSla((prev) => ({ ...prev, followUpEnabled: !prev.followUpEnabled }))}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${routingSla.followUpEnabled ? 'bg-cyan-600' : 'bg-slate-700'}`}
                role="switch"
                aria-checked={routingSla.followUpEnabled}
              >
                <span className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${routingSla.followUpEnabled ? 'translate-x-6' : 'translate-x-1'}`} />
              </button>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block text-xs text-slate-400">
                ...
                <input
                  ...
                  min={5}
                  max={1440}
                  value={routingSla.followUpMinutes}
                  onChange={(e) => setRoutingSla((prev) => ({ ...prev, followUpMinutes: Number(e.target.value) }))}
                  className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/40"
                />
              </label>
              <label className="block text-xs text-slate-400">
                Warning minutes
                <input
                  ...
                  min={5}
                  max={1440}
                  value={routingSla.warningMinutes}
                  onChange={(e) => setRoutingSla((prev) => ({ ...prev, warningMinutes: Number(e.target.value) }))}
                  className="mt-1 w-full rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/40"
                />
              </label>
            </div>
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-h-5 text-sm">
            {routingLoading && <span className="text-slate-500">Loading settings...</span>}
            {routingNotice && <span className="text-emerald-300">{routingNotice}</span>}
            {routingError && <span className="text-rose-300">{routingError}</span>}
          </div>
          <Button onClick={saveRoutingSlaSettings} disabled={routingSaving || routingLoading} className="bg-cyan-600 hover:bg-cyan-500">
            {routingSaving ? 'Saving...' : 'Save Routing + SLA'}
          </Button>
        </div>
      </Card>
        ...
        {mergeMode && (
          <div className="mt-4 p-4 rounded-xl bg-blue-500/10 border border-blue-500/30">
            <div className="flex items-start gap-3">
              <Shuffle className="mt-0.5 h-6 w-6 text-blue-300" />
                      ...
                      <div className="flex items-center gap-2">
                        {lead.phone && (
                          <a
                            ...
                            className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-500/10 border border-emerald-400/30 text-emerald-200 text-[10px] font-semibold hover:bg-emerald-500/20"
                          >
                        ...
                        {lead.email && (
                          <a
                            ...
                            onClick={(e) => e.stopPropagation()}
                            className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-purple-500/10 border border-purple-400/30 text-purple-200 text-[10px] font-semibold hover:bg-purple-500/20"
                          >
