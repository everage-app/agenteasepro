...
import { AlertTriangle, CheckCircle2, Database, FileArchive, FolderInput, Handshake, Home, UsersRound } from 'lucide-react';
...
Bob,Johnson,bob.johnson@email.com,(801) 555-0125,BOTH,Open House,Active buyer and seller`;
  ...
  return (
    <div className="space-y-6">
      ...
      {message && (
        (() => {
          const MessageIcon = message.type === 'success' ? CheckCircle2 : AlertTriangle;
          return (
        ...
        {importStep === 'upload' && (
          <>
            <div className="rounded-xl border border-dashed border-white/20 bg-white/5 p-8 text-center">
              <FolderInput className="mx-auto mb-3 h-10 w-10 text-cyan-300" />
          ...
          <div className="space-y-4">
            <div className="bg-green-500/10 border border-green-400/30 rounded-lg p-4">
              ...
              <div className="space-y-1 text-xs text-slate-300">
                <p className="inline-flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-300" /> {importResult.success} clients imported successfully</p>
                {importResult.skipped > 0 && (
                  <p className="inline-flex items-center gap-1 text-yellow-400"><AlertTriangle className="h-3.5 w-3.5" /> {importResult.skipped} duplicates skipped</p>
                )}
                {importResult.errors.length > 0 && (
                  <p className="inline-flex items-center gap-1 text-red-400"><AlertTriangle className="h-3.5 w-3.5" /> {importResult.errors.length} rows had errors</p>
                )}
              </div>
            </div>
      ...
      <Card className="p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-cyan-400/30 bg-cyan-500/5 p-4">
            <div className="flex items-center gap-3 mb-3">
              <FileArchive className="h-6 w-6 text-cyan-300" />
              <div>
      ...
      <Card className="p-6 bg-gradient-to-br from-slate-900/80 to-slate-800/80 border-white/10">
        <div className="flex items-start gap-4">
          <Database className="mt-0.5 h-6 w-6 text-[#f2d894]" />
          <div>
