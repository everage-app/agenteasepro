...
type BrokerageLogoBackground = 'CARD' | 'TRANSPARENT';
...
const clampBrokerageLogoWidth = (value: unknown) => {
  const numericValue = Number(value);
  if (!Number.isFinite(numericValue)) return 260;
  return Math.min(420, Math.max(140, Math.round(numericValue)));
};

const normalizeBrokerageLogoBackground = (value: unknown): BrokerageLogoBackground => {
...
const writeBrandingCache = (cacheKey: string, data: BrandingCache) => {
  ...
  try {
    ...
    // Ignore cache write failures (quota, private mode, etc.) without blocking branding updates.
  }
};
...
export function BrandingSettingsPage() {
  ...
  const [brokerageLogoWidth, setBrokerageLogoWidth] = useState(260);
  const [brokerageLogoBackground, setBrokerageLogoBackground] = useState<BrokerageLogoBackground>('CARD');
  ...
  const applyBrandingState = (values: Partial<BrandingCache>) => {
    setLogoUrl(normalizeAssetUrl(values.logoUrl));
  ...
  const applyBrokerageState = (values: Partial<BrokerageProfileData['settings']>) => {
    setProfilePhotoUrl(normalizeAssetUrl(values.photoUrl));
    setProfileFirstName(values.firstName ?? '');
    setProfileLastName(values.lastName ?? '');
    setBrokerageName(values.brokerageName ?? '');
    setBrokerageLogoUrl(normalizeAssetUrl(values.brokerageLogoUrl));
    setBrokerageLogoWidth(clampBrokerageLogoWidth(values.brokerageLogoWidth));
    setBrokerageLogoBackground(normalizeBrokerageLogoBackground(values.brokerageLogoBackground));
    ...
    const loadBrokerage = async () => {
      try {
        ...
        applyBrokerageState({
          ...
          brokerageLogoWidth: settings.brokerageLogoWidth ?? 260,
          brokerageLogoBackground: settings.brokerageLogoBackground || 'CARD',
  ...
  useEffect(() => {
    const handleAssetsUpdated = (event: Event) => {
      const detail = readSettingsAssetsUpdatedDetail(event);
      ...
      if (Object.prototype.hasOwnProperty.call(detail, 'brokerageLogoUrl')) {
        setBrokerageLogoUrl(normalizeAssetUrl(detail.brokerageLogoUrl));
      }

      if (Object.prototype.hasOwnProperty.call(detail, 'brokerageName')) {
        setBrokerageName(detail.brokerageName || '');
      }

      if (Object.prototype.hasOwnProperty.call(detail, 'brokerageLogoWidth')) {
        setBrokerageLogoWidth(clampBrokerageLogoWidth(detail.brokerageLogoWidth));
      }

      if (Object.prototype.hasOwnProperty.call(detail, 'brokerageLogoBackground')) {
        setBrokerageLogoBackground(normalizeBrokerageLogoBackground(detail.brokerageLogoBackground));
      }
    };

    window.addEventListener(SETTINGS_ASSETS_UPDATED_EVENT, handleAssetsUpdated);
    return () => window.removeEventListener(SETTINGS_ASSETS_UPDATED_EVENT, handleAssetsUpdated);
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let refreshedProfilePhotoUrl = profilePhotoUrl;
      let refreshedBrokerageLogoUrl = brokerageLogoUrl;
      let refreshedBrokerageName = brokerageName;
      let refreshedBrokerageLogoWidth = brokerageLogoWidth;
      let refreshedBrokerageLogoBackground = brokerageLogoBackground;

      if (reloadProfileResponse.ok) {
        const data: BrokerageProfileData = await reloadProfileResponse.json();
        const settings = data.settings || ({} as BrokerageProfileData['settings']);
        refreshedProfilePhotoUrl = normalizeAssetUrl(settings.photoUrl);
        refreshedBrokerageLogoUrl = normalizeAssetUrl(settings.brokerageLogoUrl);
        refreshedBrokerageName = settings.brokerageName || data.brokerageName || '';
        refreshedBrokerageLogoWidth = clampBrokerageLogoWidth(settings.brokerageLogoWidth);
        refreshedBrokerageLogoBackground = normalizeBrokerageLogoBackground(settings.brokerageLogoBackground);
        applyBrokerageState({
          firstName: settings.firstName || agent?.name?.split(' ')[0] || '',
          lastName: settings.lastName || agent?.name?.split(' ').slice(1).join(' ') || '',
          photoUrl: settings.photoUrl || null,
      ...
      dispatchSettingsAssetsUpdated({
        profilePhotoUrl: refreshedProfilePhotoUrl,
        brokerageLogoUrl: refreshedBrokerageLogoUrl,
        brokerageName: refreshedBrokerageName,
        brokerageLogoWidth: refreshedBrokerageLogoWidth,
        brokerageLogoBackground: refreshedBrokerageLogoBackground,
      });
  ...
  const handleBrokerageLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const freshLogoUrl = withAssetCacheBust(data.brokerageLogoUrl) || normalizeAssetUrl(data.brokerageLogoUrl);
      setBrokerageLogoUrl(freshLogoUrl);
      dispatchSettingsAssetsUpdated({ brokerageLogoUrl: freshLogoUrl });
  ...
  const handleRemoveBrokerageLogo = async () => {
    try {
      ...
      dispatchSettingsAssetsUpdated({ brokerageLogoUrl: null });
  ...
  const brokerageLogoPreviewWidth = clampBrokerageLogoWidth(brokerageLogoWidth);
  const brokerageLogoFrameClass = brokerageLogoBackground === 'TRANSPARENT'
    ? 'border-transparent bg-transparent shadow-none'
    : 'border-slate-200 bg-white shadow-[0_18px_45px_-28px_rgba(15,23,42,0.7)] dark:border-white/10 dark:bg-white/[0.06]';
  ...
  return (
    <div className="space-y-6">
      ...
      {saveMessage && (
        (() => {
          const ToastIcon = saveMessage.type === 'success' ? CheckCircle2 : AlertTriangle;
          return (
        <div className="fixed bottom-6 right-6 z-[9999]">
          ...
          >
            <ToastIcon className="h-5 w-5" strokeWidth={2.2} />
      ...
      <Card className="p-6">
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-[#d6b56d]/25 bg-[#d6b56d]/10 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#f2d894]">
            ...
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-start">
              <div>
                <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-white">
                  <ImageIcon className="h-4 w-4 text-[#7a5a24] dark:text-[#f2d894]" />
                  Brokerage logo preview
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white/75 p-4 dark:border-white/10 dark:bg-slate-950/35">
                  <div className="flex min-h-[116px] items-center justify-center overflow-hidden rounded-xl border border-dashed border-slate-300/80 bg-slate-100/60 px-3 py-4 dark:border-white/15 dark:bg-slate-950/45">
                    <div
                      className={`flex h-24 max-w-full items-center justify-center rounded-2xl border px-4 py-3 transition-all duration-200 ${brokerageLogoFrameClass}`}
                      style={{ width: brokerageLogoPreviewWidth }}
                    >
                      {brokerageLogoUrl ? (
                        <img
                          ...
                          onError={() => setBrokerageLogoUrl(null)}
                        />
                      ) : (
                        <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400">
                          <ImageOff className="h-5 w-5" />
                          <span className="text-xs font-medium">No brokerage logo uploaded</span>
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="mt-3 min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
                      {brokeragePhone && <span>{brokeragePhone}</span>}
                      {brokerageAddress && <span className="truncate">{brokerageAddress}</span>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <label
                    ...
                    className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-[#d6b56d]/35 bg-[#d6b56d]/10 px-3 py-2 text-sm font-semibold text-[#f2d894] transition-colors hover:bg-[#d6b56d]/20"
                  >
                  ...
                  {brokerageLogoUrl && (
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm font-semibold text-red-300 transition-colors hover:bg-red-500/20"
                    >
                ...
                <div>
                  ...
                  <input
                    ...
                    max={420}
                    step={10}
                    value={brokerageLogoPreviewWidth}
                    onChange={(e) => setBrokerageLogoWidth(clampBrokerageLogoWidth(e.target.value))}
                    className="w-full accent-[#d6b56d]"
                  />
                  <div className="mt-1 flex justify-between text-[10px] uppercase tracking-[0.12em] text-slate-500">
                    <span>Compact</span>
                ...
                <div>
                  <div className="mb-2 text-sm font-medium text-slate-300">Logo background</div>
                  <div className="grid grid-cols-2 gap-2 rounded-xl border border-white/10 bg-white/5 p-1">
                    <button
                      type="button"
                      onClick={() => setBrokerageLogoBackground('CARD')}
                      className={`inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition-colors ${
                        brokerageLogoBackground === 'CARD'
                          ? 'bg-white text-slate-900 shadow-sm dark:bg-[#f2d894] dark:text-[#171106]'
                    ...
                    <button
                      type="button"
                      onClick={() => setBrokerageLogoBackground('TRANSPARENT')}
                      className={`inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition-colors ${
                        brokerageLogoBackground === 'TRANSPARENT'
                          ? 'bg-white text-slate-900 shadow-sm dark:bg-[#f2d894] dark:text-[#171106]'
                          : 'text-slate-300 hover:bg-white/10 hover:text-white'
                      }`}
                    >
