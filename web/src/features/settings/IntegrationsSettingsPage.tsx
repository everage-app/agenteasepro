...
export function IntegrationsSettingsPage() {
  ...
  const readinessChannels = channels.filter((channel) => ['WEBSITE', 'EMAIL', 'SMS', 'FACEBOOK', 'INSTAGRAM', 'LINKEDIN'].includes(channel.type));
  const averageReadiness = readinessChannels.length
    ? Math.round(readinessChannels.reduce((sum, channel) => sum + (channel.readiness?.score || 0), 0) / readinessChannels.length)
    : 0;
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6 border border-emerald-400/20 bg-slate-950/50">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="h-5 w-5 text-emerald-300" />
              <h2 className="text-lg font-semibold text-slate-50">Integration Readiness</h2>
            </div>
            <p className="mt-1 text-sm text-slate-400">Launch checks for capture, follow-up, sender health, and connected tools.</p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold text-emerald-200">{averageReadiness}%</div>
            <div className="text-xs uppercase tracking-wider text-slate-500">Average ready</div>
          </div>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {readinessChannels.map((channel) => {
            const readiness = channel.readiness || { score: 0, checks: [] };
            const connected = channel.status === 'connected';
            return (
              <div key={channel.type} className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-slate-100">{channel.type.replace(/_/g, ' ')}</div>
                    <div className="text-xs text-slate-500">{connected ? channel.displayName || 'Connected' : 'Not connected'}</div>
                  </div>
                  ...
                  {readiness.checks.slice(0, 4).map((check) => (
                    <div key={check.label} className="flex items-center justify-between gap-2 text-xs">
                      <span className="inline-flex min-w-0 items-center gap-1.5 text-slate-300">
                        {check.ok ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-emerald-300" /> : <XCircle className="h-3.5 w-3.5 shrink-0 text-slate-500" />}
                        <span className="truncate">{check.label}</span>
                      </span>
                      {check.detail && <span className="shrink-0 text-slate-500">{check.detail}</span>}
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  {landingPages.length > 0 ? (
                    ...
                    <div className="text-xs text-slate-500">
                      ...
                      <a href="/landing-pages" className="text-cyan-400 hover:underline">
                        Create one →
                      </a>
                    </div>
                  )}
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-start gap-4">
                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${integration.color} border text-2xl`}>
                  <IntegrationIcon className="h-6 w-6 text-slate-100" />
                </div>
                <div>
                  <div className="flex items-center gap-3 mb-1">
