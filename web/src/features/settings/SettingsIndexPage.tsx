...
export function SettingsIndexPage() {
  ...
  useEffect(() => {
    const loadProgress = async () => {
      try {
        ...
        let hasLeadRouting = false;
        try {
          const routingRes = await fetch('/api/leads/settings/routing-sla', {
            headers: { Authorization: `Bearer ${authToken}` },
          });
          const routingData = routingRes.ok ? await routingRes.json() : null;
          hasLeadRouting = Boolean(routingData?.followUpEnabled && routingData?.followUpMinutes && routingData?.routingMode);
        } catch {}
        ...
        setProgress({
          ...
          leadRouting: hasLeadRouting,
          trust: Boolean(hasProfile && hasLicense && hasNotifications && hasIntegrations),
        });
      } catch (error) {
  ...
  const steps = [
    ...
    },
    {
      key: 'leadRouting',
      label: 'Lead routing',
      description: 'Set assignment and first-response SLA',
      href: '/settings/leads',
      icon: Route,
    },
    {
      key: 'trust',
      label: 'Trust checks',
      description: 'Confirm identity, license, alerts, and capture',
      href: '/settings/profile',
      icon: ShieldCheck,
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-slate-900/60">
        <div className="flex items-start gap-4">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-amber-300/25 bg-amber-500/12 text-amber-200">
            <Lightbulb className="h-5 w-5" strokeWidth={2.1} />
          </div>
          <div>
            ...
            <button
              ...
              className="inline-flex items-center gap-1 text-xs text-cyan-400 hover:text-cyan-300 font-medium underline"
            >
              Browse help center <ChevronRight className="h-3 w-3" />
            </button>
          </div>
        </div>
      </Card>
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white/95 dark:bg-slate-950/90 border-l border-slate-200/80 dark:border-white/10 p-6 overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-white"
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5">
                ...
                >
                  <span className="inline-flex items-center gap-1">Complete profile <ChevronRight className="h-3 w-3" /></span>
                </button>
              </div>
              <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5">
                ...
                <button
                  ...
                  className="mt-3 text-xs text-cyan-700 hover:text-cyan-600 dark:text-cyan-300 dark:hover:text-cyan-200"
                >
                  <span className="inline-flex items-center gap-1">Go to integrations <ChevronRight className="h-3 w-3" /></span>
                </button>
              </div>
              <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5">
                ...
                <button
                  ...
                  className="mt-3 text-xs text-cyan-700 hover:text-cyan-600 dark:text-cyan-300 dark:hover:text-cyan-200"
                >
                  <span className="inline-flex items-center gap-1">Configure automations <ChevronRight className="h-3 w-3" /></span>
                </button>
              </div>
            </div>
          </div>
