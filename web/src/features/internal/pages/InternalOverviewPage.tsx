...
type OverviewResponse = {
  ...
  briefing: {
    ...
    queues: {
      churnRisk: Array<{
        agent: { id: string; name: string; email: string; brokerageName: string | null; subscriptionStatus: string };
        healthScore: number;
        riskLevel: 'low' | 'medium' | 'high' | 'critical';
        reasons: string[];
        nextBestAction: string;
      }>;
      billingRecovery: Array<{
        agent: { id: string; name: string; email: string; subscriptionStatus: string };
        healthScore: number;
        riskLevel: 'low' | 'medium' | 'high' | 'critical';
        nextBestAction: string;
      }>;
      urgentSupport: Array<{
        ...
        createdAt: string;
        dueAt?: string | null;
        agent?: { id: string; name: string; email: string; brokerageName: string | null };
        assignedToAgent?: { id: string; name: string; email: string } | null;
      }>;
    };
    audit: Array<{
      id: string;
...
function badgeClassForRisk(riskLevel: 'low' | 'medium' | 'high' | 'critical') {
  switch (riskLevel) {
    case 'critical':
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
            <Card
              ...
              description="Launch-day priorities, health snapshot, and recent internal actions."
              hover={false}
              className="xl:col-span-4"
            >
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="text-[11px] uppercase tracking-wider text-slate-500">Avg health</div>
                  <div className="mt-2 text-2xl font-semibold text-white">{data.briefing.kpis.avgHealth}</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="text-[11px] uppercase tracking-wider text-slate-500">High risk</div>
                  <div className="mt-2 text-2xl font-semibold text-rose-200">{data.briefing.kpis.highRiskAgents}</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="text-[11px] uppercase tracking-wider text-slate-500">Open support</div>
                  <div className="mt-2 text-2xl font-semibold text-amber-200">{data.briefing.kpis.openSupport}</div>
                  <div className="mt-1 text-[11px] text-slate-500">{data.briefing.kpis.overdueSupport} overdue</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="text-[11px] uppercase tracking-wider text-slate-500">Internal staff</div>
                  <div className="mt-2 text-2xl font-semibold text-cyan-100">{data.briefing.kpis.internalStaff}</div>
                  <div className="mt-1 text-[11px] text-slate-500">{data.briefing.kpis.systemErrors24h} errors in 24h</div>
                </div>
              </div>

              <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="text-[11px] uppercase tracking-wider text-slate-500">Recommended next moves</div>
                <div className="mt-3 space-y-2">
                  {data.briefing.recommendedActions.length === 0 ? (
                    <div className="text-sm text-slate-400">No urgent platform-wide actions right now.</div>
                  ) : (
                    data.briefing.recommendedActions.map((action) => (
                      <div key={action} className="rounded-xl border border-cyan-500/20 bg-cyan-500/10 px-3 py-2 text-sm text-cyan-100">
                        {action}
                      </div>
                    ))
                  )}
              ...
              <div className="mt-5">
                <div className="mb-3 text-[11px] uppercase tracking-wider text-slate-500">Recent admin actions</div>
                <div className="space-y-2">
                  {data.briefing.audit.slice(0, 4).map((row) => (
                    <div key={row.id} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                      <div className="text-sm font-medium text-white">{row.summary}</div>
                      <div className="mt-1 text-[11px] text-slate-400">
                        {row.actor?.name || row.actor?.email || 'System'} · {new Date(row.createdAt).toLocaleString()}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Card
              ...
              description="Accounts that need direct human intervention next."
              hover={false}
              className="xl:col-span-4"
            >
              <div className="space-y-3">
                {data.briefing.queues.churnRisk.length === 0 ? (
                  <div className="text-sm text-slate-400">No high-risk accounts right now.</div>
                ) : (
                  data.briefing.queues.churnRisk.slice(0, 5).map((profile) => (
                    <button
                      key={profile.agent.id}
                      type="button"
                      onClick={() => navigate(`/internal/agents/${profile.agent.id}`)}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-sm font-semibold text-white truncate">{profile.agent.name}</div>
                          <div className="text-xs text-slate-400 truncate">{profile.agent.email}</div>
                        </div>
                        <div className="text-right">
                          <Badge className={badgeClassForRisk(profile.riskLevel)}>{profile.riskLevel}</Badge>
                          <div className="mt-2 text-xs text-slate-500">Health {profile.healthScore}</div>
                        </div>
                      </div>
                      <div className="mt-3 text-xs text-slate-300">{profile.reasons[0] || profile.nextBestAction}</div>
                      <div className="mt-2 text-[11px] text-cyan-200">{profile.nextBestAction}</div>
                    </button>
                  ))
                )}
              </div>
            ...
            <Card
              title="Ops Queues"
              description="Billing follow-up and urgent support workload."
              hover={false}
              className="xl:col-span-4"
            >
              <div>
                <div className="mb-3 flex items-center justify-between">
                  <div className="text-[11px] uppercase tracking-wider text-slate-500">Billing recovery</div>
                  <button
                    type="button"
                ...
                <div className="space-y-2">
                  {data.briefing.queues.billingRecovery.slice(0, 3).map((profile) => (
                    <button
                      key={profile.agent.id}
                      type="button"
                      onClick={() => navigate(`/internal/agents/${profile.agent.id}`)}
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-white truncate">{profile.agent.name}</div>
                          <div className="text-[11px] text-slate-400 truncate">{profile.agent.subscriptionStatus}</div>
                        </div>
                        <Badge className={badgeClassForRisk(profile.riskLevel)}>{profile.riskLevel}</Badge>
                      </div>
                    </button>
                  ))}
                  {data.briefing.queues.billingRecovery.length === 0 && (
                    <div className="text-sm text-slate-400">No accounts are waiting on billing follow-up.</div>
                  )}
                </div>
              </div>
              ...
              <div className="mt-5 border-t border-white/10 pt-5">
                <div className="mb-3 flex items-center justify-between">
                  <div className="text-[11px] uppercase tracking-wider text-slate-500">Urgent support</div>
                  <button
                    type="button"
                ...
                <div className="space-y-2">
                  {data.briefing.queues.urgentSupport.slice(0, 3).map((ticket) => (
                    <button
                      key={ticket.id}
                      type="button"
                      onClick={() => navigate('/internal/support')}
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px] text-slate-400 truncate">
                            {ticket.agent?.name || ticket.agent?.email || 'Unknown agent'}
                            {ticket.assignedToAgent ? ` · ${ticket.assignedToAgent.name || ticket.assignedToAgent.email}` : ' · Unassigned'}
                          </div>
                        </div>
                        <div className="text-right">
                          <Badge className={ticket.priority === 'URGENT' ? 'bg-rose-500/15 text-rose-200 border-rose-400/30' : 'bg-amber-500/15 text-amber-200 border-amber-400/30'}>
                            {ticket.priority}
                          </Badge>
                          <div className="mt-2 text-[11px] text-slate-500">{new Date(ticket.createdAt).toLocaleDateString()}</div>
                        </div>
                      </div>
                    </button>
                  ))}
                  {data.briefing.queues.urgentSupport.length === 0 && (
                    <div className="text-sm text-slate-400">No urgent support items are open.</div>
                  )}
                </div>
              </div>
            </Card>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-full border border-slate-400/30 bg-white/5 px-2 py-1 text-[10px] font-semibold text-slate-200 hover:bg-white/10"
                      >
                      ...
                      <a
                        ...
                        className="inline-flex items-center gap-1 rounded-full border border-emerald-400/30 bg-emerald-500/10 px-2 py-1 text-[10px] font-semibold text-emerald-200 hover:bg-emerald-500/20"
                      >
