              ...
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
                ...
                <div className="xl:col-span-5">
                  <div className="space-y-5">
                    ...
                    <Card title="Billing recovery queue" description="Accounts that need billing or trial follow-up next." hover={false}>
                      <div className="space-y-2">
                        {briefing?.queues?.billingRecovery?.length ? (
                          briefing.queues.billingRecovery.slice(0, 5).map((item) => (
                            <button
                              key={item.agent.id}
                              type="button"
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  <div className="text-sm font-semibold text-white truncate">{item.agent.name}</div>
                                  <div className="text-[11px] text-slate-500 truncate">{item.agent.subscriptionStatus}</div>
                                </div>
                                <div className="text-right">
                                  <Badge className={riskBadgeClass(item.riskLevel)}>{item.riskLevel}</Badge>
                                  <div className="mt-2 text-[11px] text-slate-500">Health {item.healthScore}</div>
                                </div>
                              </div>
                              <div className="mt-2 text-[11px] text-cyan-200 line-clamp-2">{item.nextBestAction}</div>
                            </button>
                          ))
                        ) : (
                          <div className="text-sm text-slate-400">No accounts currently need billing recovery follow-up.</div>
                        )}
                      </div>
                    </Card>
                  </div>
                          ...
                          <div className="min-w-0">
                            ...
                            {agent.health && (
                              <div className="mt-1 flex items-center gap-2">
                                <Badge className={riskBadgeClass(agent.health.riskLevel)}>{agent.health.riskLevel}</Badge>
                                <span className="text-[11px] text-slate-500">Health {agent.health.healthScore}</span>
                              </div>
                            )}
                          </div>
                        </div>
            ...
            <Card title="Billing workspace" description="Review live subscription details" hover={false}>
              {subLoading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div className="text-xs uppercase tracking-[0.2em] text-slate-500">Health context</div>
                    {selectedAgent?.health ? (
                      <>
                        <div className="mt-2 flex items-center gap-2">
                          <Badge className={riskBadgeClass(selectedAgent.health.riskLevel)}>{selectedAgent.health.riskLevel}</Badge>
                          <span className="text-sm font-semibold text-white">Health {selectedAgent.health.healthScore}</span>
                        </div>
                        <div className="mt-3 text-xs text-cyan-200 line-clamp-3">{selectedAgent.health.nextBestAction}</div>
                      </>
                    ) : (
                      <div className="mt-2 text-xs text-slate-400">No health summary available for this account.</div>
                    )}
                  </div>
                </div>
              )}
