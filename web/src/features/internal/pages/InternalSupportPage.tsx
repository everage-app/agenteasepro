...
type SupportRequestRow = {
  ...
  assignedToAgent?: AgentLite | null;
  dueAt?: string | null;
...
export function InternalSupportPage() {
  ...
  const [sla, setSla] = useState<SupportSlaSummary>({ overdue: 0, urgent: 0, unassigned: 0 });
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setSla(res.data?.sla ?? { overdue: 0, urgent: 0, unassigned: 0 });
  ...
  const summary = useMemo(() => {
    return {
      ...
      overdue: sla.overdue || 0,
      urgent: sla.urgent || 0,
      unassigned: sla.unassigned || 0,
    };
  }, [counts, sla]);

  const assignees = useMemo(() => {
    return staff.filter((row) => row.role !== 'READ_ONLY' && row.role !== 'BILLING' && row.role !== 'SALES');
  }, [staff]);

  const updateRequest = async (id: string, payload: Partial<SupportRequestRow> & { assignedToAgentId?: string | null; reason?: string }) => {
    ...
    >
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-5">
        <Card className="p-4" title="Open">
          ...
          <div className="mt-1 text-[11px] text-slate-500">{summary.urgent} urgent</div>
        </Card>
        <Card className="p-4" title="In progress">
          <div className="text-2xl font-bold text-white">{summary.inProgress}</div>
        </Card>
        <Card className="p-4" title="Overdue">
          <div className="text-2xl font-bold text-rose-200">{summary.overdue}</div>
          <div className="mt-1 text-[11px] text-slate-500">Open or in progress past SLA</div>
        </Card>
        <Card className="p-4" title="Unassigned">
          <div className="text-2xl font-bold text-amber-100">{summary.unassigned}</div>
          <div className="mt-1 text-[11px] text-slate-500">Needs an owner</div>
        </Card>
        <Card className="p-4" title="Resolved / closed">
          <div className="text-2xl font-bold text-white">{summary.resolved + summary.closed}</div>
          <div className="mt-1 text-[11px] text-slate-500">Historical completions</div>
        </Card>
      </div>
              ...
              {requests.map((row) => {
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
                          ...
                          <span>•</span>
                          <span>{row.assignedToAgent ? `Assigned to ${row.assignedToAgent.name || row.assignedToAgent.email}` : 'Unassigned'}</span>
                          {row.dueAt ? (
                            <>
                              <span>•</span>
                              <span className={new Date(row.dueAt).getTime() < Date.now() && row.status !== 'RESOLVED' && row.status !== 'CLOSED' ? 'text-rose-300' : ''}>
                    ...
                    {isExpanded && (
                      <div className="mt-3 space-y-3 text-xs text-slate-300" onClick={(e) => e.stopPropagation()}>
                        <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                          ...
                          {row.firstResponseAt && (
                            <div className="mt-1 text-[11px] text-slate-400">First response: {new Date(row.firstResponseAt).toLocaleString()}</div>
                          )}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 items-center">
                          ...
                          <select
                            value={row.assignedToAgentId || ''}
                            onChange={(e) => updateRequest(row.id, { assignedToAgentId: e.target.value || null })}
                            className="px-3 py-2 rounded-xl text-xs bg-white/5 border border-white/10 text-slate-200"
                          >
                            <option value="">Unassigned</option>
                            {assignees.map((assignee) => (
                              <option key={assignee.agent.id} value={assignee.agent.id}>
                                {(assignee.agent.name || assignee.agent.email) + ` · ${assignee.role}`}
                              </option>
                            ))}
                          </select>
                          {row.pageUrl && (
