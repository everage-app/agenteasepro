...
type DealFocusFilter = 'ALL' | 'URGENT' | 'DOCS' | 'STALE' | 'NEEDS_REPC';
...
function getPartyName(person?: { firstName?: string; lastName?: string } | null) {
...
function daysSinceDealActivity(deal: DealCard) {
  const value = deal.lastActivityAt || deal.closedAt || deal.offerReferenceDate;
...
function formatDealActivityAge(deal: DealCard) {
  const days = daysSinceDealActivity(deal);
  if (!Number.isFinite(days)) return 'No activity logged';
  if (days <= 0) return 'Updated today';
  if (days === 1) return 'Updated yesterday';
  return `Updated ${days}d ago`;
}

function needsRepc(deal: DealCard) {
  return !deal.repc && ['OFFER_SENT', 'UNDER_CONTRACT', 'DUE_DILIGENCE', 'FINANCING', 'SETTLEMENT_SCHEDULED'].includes(deal.status);
}

function getDealNextMove(deal: DealCard) {
  const nextDeadline = getNextDeadline(deal);
  const urgency = getUrgencyLevel(nextDeadline?.date || null);
  const docs = getDealDocsSummary(deal);
  const staleDays = daysSinceDealActivity(deal);

  if (urgency === 'critical' && nextDeadline) {
    return {
      label: `Handle ${nextDeadline.label}`,
      detail: nextDeadline.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
  ...
  if (needsRepc(deal)) {
    return { label: 'Create or attach REPC', detail: 'Contract milestone has no REPC', tone: 'urgent' as const, icon: FileSignature };
  }

  if (docs.pending > 0) {
    return { label: 'Finish signatures', detail: `${docs.pending} pending document${docs.pending === 1 ? '' : 's'}`, tone: 'warning' as const, icon: FileSignature };
  }

  if (urgency === 'warning' && nextDeadline) {
    return {
      label: `Prepare ${nextDeadline.label}`,
      detail: nextDeadline.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
  ...
  if (!deal.buyer && !deal.seller) {
    return { label: 'Assign client parties', detail: 'Buyer or seller missing', tone: 'warning' as const, icon: AlertTriangle };
  }

  if (!Number.isFinite(staleDays) || staleDays >= 7) {
    return { label: 'Refresh deal update', detail: formatDealActivityAge(deal), tone: 'strong' as const, icon: Clock3 };
  }

  return { label: 'Move to next milestone', detail: formatDealActivityAge(deal), tone: 'success' as const, icon: MoveRight };
}

function dealNextMoveClass(tone: ReturnType<typeof getDealNextMove>['tone']) {
...
function matchesDealFocus(filter: DealFocusFilter, deal: DealCard) {
  if (filter === 'ALL') return true;
  if (filter === 'URGENT') {
    const nextDeadline = getNextDeadline(deal);
    return getUrgencyLevel(nextDeadline?.date || null) !== 'normal';
  }
  if (filter === 'DOCS') return getDealDocsSummary(deal).pending > 0;
  if (filter === 'STALE') return daysSinceDealActivity(deal) >= 7;
  if (filter === 'NEEDS_REPC') return needsRepc(deal);
  return true;
}

function focusFilterLabel(filter: DealFocusFilter) {
  const labels: Record<DealFocusFilter, string> = {
    ALL: 'All active deals',
    URGENT: 'Deadline risk',
    DOCS: 'Documents pending',
    STALE: 'Stale updates',
    NEEDS_REPC: 'Needs REPC',
  };
  return labels[filter];
}

function DealFocusButton({
  active,
  icon,
  label,
  count,
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-bold transition-colors ${
        active
          ? 'border-[#f2d894]/45 bg-[#d6b56d]/20 text-[#f7e7b0]'
          : 'border-white/10 bg-white/5 text-slate-300 hover:border-[#f2d894]/25 hover:bg-white/10 hover:text-white'
      }`}
    >
      {icon}
      <span>{label}</span>
...
export function DealsKanban() {
  ...
  const openManageDeal = (deal: DealCard) => {
    ...
    setEditForm({
      title: cleanDisplayText(deal.title),
      status: deal.status,
      ...
      propertyStreet: cleanDisplayText(deal.property?.street),
      propertyCity: cleanDisplayText(deal.property?.city),
      propertyState: cleanDisplayText(deal.property?.state),
      propertyZip: cleanDisplayText(deal.property?.zip),
      propertyCounty: cleanDisplayText(deal.property?.county),
      propertyTaxId: cleanDisplayText(deal.property?.taxId),
      propertyMlsId: cleanDisplayText(deal.property?.mlsId),
      buyerFirstName: cleanDisplayText(deal.buyer?.firstName),
      buyerLastName: cleanDisplayText(deal.buyer?.lastName),
      buyerEmail: cleanDisplayText(deal.buyer?.email),
      buyerPhone: cleanDisplayText(deal.buyer?.phone),
      sellerFirstName: cleanDisplayText(deal.seller?.firstName),
      sellerLastName: cleanDisplayText(deal.seller?.lastName),
      sellerEmail: cleanDisplayText(deal.seller?.email),
      sellerPhone: cleanDisplayText(deal.seller?.phone),
    });
  ...
  const visibleActiveDeals = activeDeals.filter((deal) => matchesDealFocus(dealFocusFilter, deal));
  const closedDeals = deals.filter(
    (d) => !d.archivedAt && (d.status === 'CLOSED' || d.status === 'FELL_THROUGH')
  );
  const archivedDeals = deals.filter((d) => Boolean(d.archivedAt));
  const dealFocusCounts = {
    all: activeDeals.length,
    urgent: activeDeals.filter((deal) => matchesDealFocus('URGENT', deal)).length,
    docs: activeDeals.filter((deal) => matchesDealFocus('DOCS', deal)).length,
    stale: activeDeals.filter((deal) => matchesDealFocus('STALE', deal)).length,
    needsRepc: activeDeals.filter((deal) => matchesDealFocus('NEEDS_REPC', deal)).length,
  };
  ...
  const dealMoveRank = { urgent: 0, warning: 1, strong: 2, success: 3 } as const;
  const priorityDeal = [...activeDeals].sort((left, right) => {
    const leftMove = getDealNextMove(left);
    const rightMove = getDealNextMove(right);
    return dealMoveRank[leftMove.tone] - dealMoveRank[rightMove.tone] || daysSinceDealActivity(right) - daysSinceDealActivity(left);
  })[0] || null;
  const priorityMove = priorityDeal ? getDealNextMove(priorityDeal) : null;
  const PriorityMoveIcon = priorityMove?.icon || MoveRight;
  ...
  return (
    <div className="space-y-5 relative">
      ...
      <div className="rounded-[24px] border border-white/10 bg-slate-950/70 px-5 py-5 md:px-6 md:py-6 shadow-[0_18px_45px_rgba(0,0,0,0.5)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 text-[11px]">
            ...
            <button
              onClick={() => setDealFocusFilter('URGENT')}
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div className="rounded-2xl border border-[#d6b56d]/35 bg-[#fff7df] px-4 py-3.5 dark:border-[#f2d894]/18 dark:bg-[#d6b56d]/10">
            {priorityDeal && priorityMove ? (
              <div className="flex min-w-0 items-start gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#d6b56d]/35 bg-white text-[#7a5a24] dark:border-[#f2d894]/25 dark:bg-black/15 dark:text-[#f7e7b0]">
                  <PriorityMoveIcon className="h-5 w-5" />
                </span>
                <div className="min-w-0">
                  <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#7a5a24] dark:text-[#f2d894]">Next best move</div>
                  <div className="mt-1 truncate text-sm font-bold text-slate-950 dark:text-white" title={priorityMove.label}>{priorityMove.label}</div>
                  <div className="mt-0.5 truncate text-xs text-slate-600 dark:text-slate-300" title={getDealDisplayTitle(priorityDeal)}>{getDealDisplayTitle(priorityDeal)} · {priorityMove.detail}</div>
                </div>
              </div>
            ) : (
              <div>
                <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#7a5a24] dark:text-[#f2d894]">Next best move</div>
                <div className="mt-1 text-sm font-bold text-slate-950 dark:text-white">Create the next active deal</div>
                <div className="mt-0.5 text-xs text-slate-600 dark:text-slate-300">The board is ready for a new client or property workflow.</div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2 lg:justify-end">
            <button
              type="button"
              onClick={() => priorityDeal ? openManageDeal(priorityDeal) : navigate('/deals/new')}
              className="inline-flex items-center gap-1.5 rounded-full border border-[#d6b56d]/35 bg-[#fff7df] px-3 py-2 text-[11px] font-bold text-[#7a5a24] transition hover:bg-[#f7e7b0]/60 dark:border-[#f2d894]/30 dark:bg-[#d6b56d]/15 dark:text-[#f7e7b0] dark:hover:bg-[#d6b56d]/25"
            >
              <MoveRight className="h-3.5 w-3.5" />
              {priorityDeal ? 'Work this deal' : 'New deal'}
            </button>
            <button
              type="button"
              onClick={() => setDealFocusFilter('DOCS')}
              className="inline-flex items-center gap-1.5 rounded-full border border-amber-400/30 bg-amber-500/10 px-3 py-2 text-[11px] font-bold text-amber-200 hover:bg-amber-500/20"
            >
            ...
            <button
              type="button"
              onClick={() => setDealFocusFilter('STALE')}
              className="inline-flex items-center gap-1.5 rounded-full border border-cyan-400/30 bg-cyan-500/10 px-3 py-2 text-[11px] font-bold text-cyan-200 hover:bg-cyan-500/20"
            >
      ...
      <div className="rounded-[24px] border border-[#f2d894]/15 bg-gradient-to-r from-[#d6b56d]/12 via-slate-950/55 to-slate-950/70 px-5 py-4 md:px-6 shadow-[0_16px_42px_rgba(0,0,0,0.45)]">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#f2d894]">Execution Queue</div>
            <div className="mt-1 text-sm font-semibold text-white">
              {focusFilterLabel(dealFocusFilter)} · {visibleActiveDeals.length} visible
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <DealFocusButton active={dealFocusFilter === 'ALL'} icon={<CheckCircle2 className="h-3.5 w-3.5" />} label="All" count={dealFocusCounts.all} onClick={() => setDealFocusFilter('ALL')} />
            <DealFocusButton active={dealFocusFilter === 'URGENT'} icon={<CalendarClock className="h-3.5 w-3.5" />} label="Deadlines" count={dealFocusCounts.urgent} onClick={() => setDealFocusFilter('URGENT')} />
            <DealFocusButton active={dealFocusFilter === 'DOCS'} icon={<FileSignature className="h-3.5 w-3.5" />} label="Docs" count={dealFocusCounts.docs} onClick={() => setDealFocusFilter('DOCS')} />
            <DealFocusButton active={dealFocusFilter === 'NEEDS_REPC'} icon={<AlertTriangle className="h-3.5 w-3.5" />} label="REPC" count={dealFocusCounts.needsRepc} onClick={() => setDealFocusFilter('NEEDS_REPC')} />
            <DealFocusButton active={dealFocusFilter === 'STALE'} icon={<Clock3 className="h-3.5 w-3.5" />} label="Stale" count={dealFocusCounts.stale} onClick={() => setDealFocusFilter('STALE')} />
          </div>
        </div>
      </div>
            ...
            <div className="flex-1 space-y-3 px-3 pb-3 pt-1 overflow-y-auto relative z-10">
              {colDeals.length === 0 ? (
                ...
                colDeals.map((deal) => {
                  ...
                  const nextMove = getDealNextMove(deal);
                  ...
                  const mapQuery = getDealMapQuery(deal);
                  return (
                  <article
                    key={deal.id}
                    role="button"
                  ...
                  >
                      ...
                      <div className="mb-2 flex flex-wrap gap-1.5">
                        ...
                        {deal.seller && (
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-400/30 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-medium text-emerald-200">
                            ...
                            {getPartyName(deal.seller) || 'Seller'}
                          </span>
                        )}
                      </div>
                    )}

                    <div className={`mb-2 rounded-xl border px-2.5 py-2 ${dealNextMoveClass(nextMove.tone)}`}>
                      <div className="flex items-start gap-2">
                        <NextMoveIcon className="mt-0.5 h-4 w-4 shrink-0" />
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-[11px] font-bold" title={nextMove.label}>{nextMove.label}</div>
                          <div className="mt-0.5 truncate text-[10px] opacity-80" title={nextMove.detail}>{nextMove.detail}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2 text-[11px]">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <FileSignature className="h-3 w-3" /> REPC
                        </button>
                        {mapQuery && (
                          <a
            ...
            <div className="flex-1 space-y-3 px-3 pb-3 pt-1 overflow-y-auto max-h-[420px] relative z-10">
              {archivedDeals.map((deal) => (
                ...
                >
                  <div className="mb-1 flex items-start justify-between gap-2">
                    <div>
                      <p className="line-clamp-2 text-[13px] font-semibold text-slate-50">{dealTitle}</p>
                      {dealAddressLine && (
                        <p className="mt-0.5 line-clamp-1 text-[11px] text-slate-400">
                          {dealAddressLine}
                        </p>
        ...
        >
          ...
          <div className="mt-3 rounded-2xl border border-cyan-400/25 bg-cyan-500/5 px-4 py-3 text-xs text-cyan-100">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(getDealMapQuery(editingDeal) || editForm.propertyStreet || editForm.title)}`}
                target="_blank"
                rel="noreferrer"
        ...
        >
          <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
            <div>
              ...
              <h3 className="text-base font-semibold text-white mt-1 line-clamp-1">{getDealDisplayTitle(activityDeal)}</h3>
            </div>
            <button
