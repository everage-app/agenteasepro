...
export function DealExecutionCockpitPage() {
  ...
  const [taskDefaultCategory, setTaskDefaultCategory] = useState<'GENERAL' | 'CONTRACT' | 'MARKETING' | 'CALL' | 'NOTE' | 'POPBY' | 'EVENT'>('CONTRACT');
  ...
  const openDealTaskModal = (preset: 'followup' | 'deadline') => {
    if (!deal) return;
    ...
    if (preset === 'deadline') {
      ...
      const due = nearest || new Date(Date.now() + 24 * 60 * 60 * 1000);
      setTaskDefaultTitle(`Deadline review: ${deal.title}`);
      setTaskDefaultCategory('CONTRACT');
      setTaskDefaultDueAt(formatDateTimeLocal(due));
      setShowTaskModal(true);
      return;
    }

    setTaskDefaultTitle(`Follow up on ${deal.title}`);
    setTaskDefaultCategory('CONTRACT');
    ...
    <PageLayout
      ...
      actions={
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button size="sm" variant="primary" onClick={() => openDealTaskModal('followup')}>
            Add deal task
          </Button>
          <Button size="sm" variant="secondary" onClick={() => openDealTaskModal('deadline')}>
            Add deadline task
          </Button>
        </div>
      }
            ...
            <Card
              ...
              headerAction={
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="secondary" onClick={() => openDealTaskModal('followup')}>
                    New task
                  </Button>
                  <Button size="sm" variant="secondary" onClick={() => openDealTaskModal('deadline')}>
                    Deadline task
