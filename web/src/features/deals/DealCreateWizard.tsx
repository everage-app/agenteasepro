...
export function DealCreateWizard() {
  ...
  useEffect(() => {
    ...
    const hasPropertyParam = Boolean(
      propertyParam ||
      mlsParam ||
    ...
    let workspaceDraft: any = null;
    try {
      const raw = sessionStorage.getItem('agentease_property_workspace_draft');
      workspaceDraft = raw ? JSON.parse(raw) : null;
    } catch {
      workspaceDraft = null;
    }

    if (!hasPropertyParam && !workspaceDraft) return;

    const nextMls = mlsParam || workspaceDraft?.mlsId || '';
    const nextStreet = searchParams.get('street') || workspaceDraft?.street || '';
    const nextCity = searchParams.get('city') || workspaceDraft?.city || '';
    const nextState = searchParams.get('state') || workspaceDraft?.state || 'UT';
    const nextZip = searchParams.get('zip') || workspaceDraft?.zip || '';
    const nextTaxId = searchParams.get('taxId') || workspaceDraft?.taxId || '';
    const rawPrice = searchParams.get('price') || workspaceDraft?.priceNumber || workspaceDraft?.price || '';
    const nextPrice = Number(String(rawPrice).replace(/[^0-9.]/g, ''));
    const normalizedCity = sanitizeCity(nextCity) || '';
    const nextCounty = normalizedCity ? CITY_TO_COUNTY[normalizedCity.toLowerCase()] || '' : '';

    setMlsQuery((prev) => prev || nextMls || propertyParam || [nextStreet, normalizedCity, nextState, nextZip].filter(Boolean).join(', '));
    if (nextMls) setMlsId(nextMls);

    setParties((prev) => ({
      ...prev,
      street: prev.street || nextStreet,
      city: prev.city || normalizedCity,
      county: prev.county || nextCounty,
      state: prev.state || nextState || 'UT',
      zip: prev.zip || nextZip,
      parcelId: prev.parcelId || nextTaxId,
    }));

    if (Number.isFinite(nextPrice) && nextPrice > 0) {
      setPrice((prev) => ({
        ...prev,
        purchasePrice: prev.purchasePrice || formatCurrency(String(Math.round(nextPrice))),
        earnestMoneyAmount: prev.earnestMoneyAmount || formatCurrency(String(Math.round(nextPrice * 0.01))),
      }));
    }
    ...
    const filled = [
      ...
      nextCounty ? 'County' : null,
      nextZip ? 'ZIP' : null,
      nextTaxId ? 'Parcel/Tax ID' : null,
      nextMls ? 'MLS #' : null,
      Number.isFinite(nextPrice) && nextPrice > 0 ? 'Purchase Price' : null,
    ].filter(Boolean);

    if (filled.length) {
      setMlsAutofillSummary(`Property workspace prefilled: ${filled.join(', ')}`);
    }
  }, [searchParams]);

  const DEAL_INITIAL_FLAT = useMemo(() => ({
    representation: '' as string,
    dealStage: 'ACTIVE' as string,
    buyerFirstName: '',
    buyerLastName: '',
    buyerEmail: '',
    sellerFirstName: '',
  ...
  const dealCurrentFlat = useMemo(() => ({
    representation: representation || '',
    dealStage,
    buyerFirstName: parties.buyerFirstName,
    buyerLastName: parties.buyerLastName,
    buyerEmail: parties.buyerEmail,
    sellerFirstName: parties.sellerFirstName,
    sellerLastName: parties.sellerLastName,
    sellerEmail: parties.sellerEmail,
    street: parties.street,
    city: parties.city,
    county: parties.county,
    zip: parties.zip,
    purchasePrice: price.purchasePrice,
    earnestMoneyAmount: price.earnestMoneyAmount,
    settlementDeadline: conditions.settlementDeadline,
  }), [representation, dealStage, parties, price, conditions]);

  const dealUnsaved = useUnsavedChanges({
    draftKey: 'deal-wizard',
    initial: DEAL_INITIAL_FLAT,
    current: dealCurrentFlat,
  });

  // Restore draft on mount
  const [dealDraftRestored, setDealDraftRestored] = useState(false);
  useEffect(() => {
    const draft = dealUnsaved.loadDraft();
    if (draft && (draft.buyerFirstName || draft.sellerFirstName || draft.street || draft.purchasePrice)) {
      if (draft.representation) setRepresentation(draft.representation as 'BUYER' | 'SELLER');
      if (draft.dealStage) setDealStage(draft.dealStage as 'ACTIVE' | 'LEAD');
      setParties((prev) => ({
        ...prev,
        buyerFirstName: draft.buyerFirstName || prev.buyerFirstName,
        buyerLastName: draft.buyerLastName || prev.buyerLastName,
        buyerEmail: draft.buyerEmail || prev.buyerEmail,
        sellerFirstName: draft.sellerFirstName || prev.sellerFirstName,
        sellerLastName: draft.sellerLastName || prev.sellerLastName,
        sellerEmail: draft.sellerEmail || prev.sellerEmail,
      ...
      setPrice((prev) => ({
        ...prev,
        purchasePrice: draft.purchasePrice || prev.purchasePrice,
        earnestMoneyAmount: draft.earnestMoneyAmount || prev.earnestMoneyAmount,
      }));
      if (draft.settlementDeadline) {
        setConditions((prev) => ({ ...prev, settlementDeadline: draft.settlementDeadline }));
      }
      setDealDraftRestored(true);
    }
  ...
  const handleClose = () => {
    if (dealUnsaved.isDirty) {
      dealUnsaved.requestClose();
    } else {
      doClose();
    }
  };
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4 overflow-y-auto">
        ...
        >
          ...
          {dealDraftRestored && step === 1 && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-blue-700 text-sm flex items-center justify-between dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-300">
              ...
              <button
                type="button"
                onClick={() => setDealDraftRestored(false)}
                className="text-xs font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 underline"
              >
                Dismiss
              </button>
            </div>
                      ...
                      {mlsResult && (
                        <div className="mt-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 p-3 text-xs text-emerald-700 dark:text-emerald-200">
                          ...
                          <div className="mt-2 flex flex-wrap gap-3 text-[11px]">
                            ...
                            {mlsResult.beds && <span className="inline-flex items-center gap-1"><BedDouble className="h-3.5 w-3.5" /> {mlsResult.beds} beds</span>}
                            {mlsResult.baths && <span className="inline-flex items-center gap-1"><Bath className="h-3.5 w-3.5" /> {mlsResult.baths} baths</span>}
                            {mlsResult.sqft && <span className="inline-flex items-center gap-1"><Ruler className="h-3.5 w-3.5" /> {mlsResult.sqft.toLocaleString()} sqft</span>}
                          </div>
  ...
  return (
    <>
      ...
      <UnsavedChangesDialog
        open={dealUnsaved.showConfirmDialog}
        onDiscard={() => {
          dealUnsaved.confirmDiscard();
          doClose();
        }}
        onCancel={dealUnsaved.cancelDiscard}
        onSaveDraft={() => {
          dealUnsaved.saveDraft();
          dealUnsaved.cancelDiscard();
          doClose();
        }}
      />
    </>
