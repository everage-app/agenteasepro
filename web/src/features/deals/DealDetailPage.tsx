...
import { AlertTriangle, CalendarClock, CalendarDays, CheckCircle2, ClipboardList, Clock3, FileSignature, FileText, MoveRight, Paperclip, PenLine } from 'lucide-react';
...
const daysSinceActivity = (deal: DealDetail) => {
...
const formatActivityAge = (deal: DealDetail) => {
...
const getDetailNextMove = (deal: DealDetail, deadlines: { label: string; date?: string | null }[], docStats: { pending: number }) => {
  const nextDeadline = deadlines
    .map((item) => ({ ...item, days: daysUntil(item.date) }))
    .filter((item) => item.days !== null)
    .sort((a, b) => (a.days ?? 999) - (b.days ?? 999))[0];

  if (nextDeadline && (nextDeadline.days ?? 999) <= 3) {
    return {
      label: `Protect ${nextDeadline.label}`,
      detail: `${formatDate(nextDeadline.date)} · ${nextDeadline.days! < 0 ? 'overdue' : `${nextDeadline.days}d`}`,
      tone: 'urgent' as const,
      icon: CalendarClock,
    };
  }

  if (!deal.repc && ['OFFER_SENT', 'UNDER_CONTRACT', 'DUE_DILIGENCE', 'FINANCING', 'SETTLEMENT_SCHEDULED'].includes(deal.status)) {
    return { label: 'Create or attach REPC', detail: 'Contract status needs paperwork', tone: 'urgent' as const, icon: FileSignature };
  }

  if (docStats.pending > 0) {
    return { label: 'Finish signatures', detail: `${docStats.pending} pending document${docStats.pending === 1 ? '' : 's'}`, tone: 'warning' as const, icon: FileSignature };
  }

  if (!deal.buyer && !deal.seller) {
    return { label: 'Assign client parties', detail: 'Buyer or seller is missing', tone: 'warning' as const, icon: AlertTriangle };
  }

  if (daysSinceActivity(deal) >= 7) {
    return { label: 'Refresh deal update', detail: formatActivityAge(deal), tone: 'strong' as const, icon: Clock3 };
  }

  return { label: 'Move to next milestone', detail: formatActivityAge(deal), tone: 'success' as const, icon: MoveRight };
};

const detailNextMoveClass = (tone: ReturnType<typeof getDetailNextMove>['tone']) => {
...
export function DealDetailPage() {
  ...
  const nextMove = getDetailNextMove(deal, deadlines, docStats);
  ...
  return (
    ...
    >
      ...
      <div className={`mb-6 rounded-2xl border px-4 py-4 shadow-[0_18px_44px_rgba(0,0,0,0.28)] ${detailNextMoveClass(nextMove.tone)}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-current/25 bg-black/15">
              <NextMoveIcon className="h-5 w-5" />
            </div>
            <div className="min-w-0">
              <div className="text-[10px] font-bold uppercase tracking-[0.18em] opacity-75">Next best move</div>
              <div className="mt-1 truncate text-sm font-bold" title={nextMove.label}>{nextMove.label}</div>
              <div className="mt-0.5 truncate text-xs opacity-80" title={nextMove.detail}>{nextMove.detail}</div>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-[11px] sm:min-w-[260px]">
            <div className="rounded-xl border border-current/15 bg-black/10 px-3 py-2">
              <div className="font-bold">{docStats.pending}</div>
              <div className="opacity-75">pending docs</div>
            </div>
            <div className="rounded-xl border border-current/15 bg-black/10 px-3 py-2">
              <div className="font-bold">{deadlines.length}</div>
              <div className="opacity-75">deadlines</div>
            </div>
            <div className="rounded-xl border border-current/15 bg-black/10 px-3 py-2">
              <div className="font-bold"><CheckCircle2 className="mx-auto h-4 w-4" /></div>
              <div className="opacity-75">{formatActivityAge(deal)}</div>
            </div>
          </div>
        </div>
      </div>
...
function ActivityTab({ items, loading }: { items: ActivityItem[]; loading: boolean }) {
  ...
  const iconFor = (type: string): LucideIcon => {
    switch (type) {
      case 'DEAL': return ClipboardList;
      case 'EVENT': return CalendarDays;
      case 'TASK': return CheckCircle2;
      case 'FORM': return FileText;
      case 'ADDENDUM': return Paperclip;
      case 'ESIGN': return PenLine;
  ...
  return (
    <div className="relative">
      ...
      <div className="space-y-0">
        {items.map(item => {
          const ActivityIcon = iconFor(item.type);
          return (
          <div key={item.id} className="relative pl-10 py-3">
            ...
            <div className="flex items-start justify-between">
              <div>
                <ActivityIcon className="mr-2 inline h-4 w-4 text-cyan-300" />
