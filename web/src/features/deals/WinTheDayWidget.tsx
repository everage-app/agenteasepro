...
export function WinTheDayWidget() {
  ...
  if (loading || !activity) {
    return (
      <div className="ae-theme-card flex h-64 w-full items-center justify-center rounded-2xl border backdrop-blur-xl sm:rounded-[32px]">
        <div className="text-sm text-slate-500 animate-pulse dark:text-slate-400">Loading today's goals...</div>
      </div>
    );
  }
  ...
  return (
    <div className="ae-theme-card relative flex w-full flex-col overflow-hidden rounded-2xl border backdrop-blur-xl sm:rounded-[32px]">
      {/* Decorative background glow */}
      <div className="pointer-events-none absolute right-0 top-0 h-64 w-64 -translate-y-1/2 translate-x-1/2 rounded-full bg-emerald-500/10 blur-[80px] dark:bg-emerald-400/10" />
      
      {/* Header */}
      <div className="relative z-10 border-b border-slate-200/80 px-6 py-4 dark:border-white/10">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-semibold text-slate-950 dark:text-slate-50">Win the Day</h3>
              <button
                onClick={() => setShowGoalSettings(true)}
                className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-slate-100 text-slate-600 transition-all duration-200 hover:bg-slate-200 hover:text-slate-950 dark:border-white/5 dark:bg-slate-800/50 dark:text-slate-400 dark:hover:bg-slate-700/50 dark:hover:text-slate-200"
                title="Adjust goals"
              >
            ...
            <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Track your daily referral & touchpoint goals</p>
          </div>
          <div className="text-right">
            <div className={`text-2xl font-bold ${isDayWon ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-800 dark:text-slate-300'}`}>
              {totalProgress}%
            </div>
            {isDayWon && (
              <div className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 dark:text-emerald-400"><Trophy className="h-3 w-3" /> Day Won!</div>
            )}
          </div>
        </div>
      </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-800 dark:text-slate-200">Calls</span>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex h-6 w-6 items-center justify-center rounded-full bg-blue-500/15 text-xs font-bold text-blue-700 transition-colors hover:bg-blue-500/25 dark:bg-blue-500/20 dark:text-blue-200 dark:hover:bg-blue-500/30"
              >
                +
              </button>
            </div>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-slate-200/90 dark:bg-black/35">
            <div
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-800 dark:text-slate-200">Notes</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-500 dark:text-slate-400">
              ...
              <button
                ...
                className="flex h-6 w-6 items-center justify-center rounded-full bg-purple-500/15 text-xs font-bold text-purple-700 transition-colors hover:bg-purple-500/25 dark:bg-purple-500/20 dark:text-purple-200 dark:hover:bg-purple-500/30"
              >
                +
              </button>
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-800 dark:text-slate-200">Pop-bys</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-500 dark:text-slate-400">
              ...
              <button
                ...
                className="flex h-6 w-6 items-center justify-center rounded-full bg-amber-500/15 text-xs font-bold text-amber-700 transition-colors hover:bg-amber-500/25 dark:bg-amber-500/20 dark:text-amber-200 dark:hover:bg-amber-500/30"
              >
                +
              </button>
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-800 dark:text-slate-200">Referrals Asked</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-500 dark:text-slate-400">
              ...
              <button
                ...
                className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500/15 text-xs font-bold text-emerald-700 transition-colors hover:bg-emerald-500/25 dark:bg-emerald-500/20 dark:text-emerald-200 dark:hover:bg-emerald-500/30"
              >
                +
              </button>
            </div>
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        ...
        >
          ...
          {/* Main Modal */}
          <div className="ae-theme-card-strong relative overflow-hidden rounded-[24px] border shadow-2xl">
            ...
            <div className="px-6 pb-4 space-y-3">
              {goalItems.map((item, index) => (
                ...
                >
                  ...
                  <div className={`ae-theme-inset relative rounded-2xl border ${item.borderColor} border-opacity-30 p-4 transition-all duration-200 hover:border-opacity-50`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-shrink-0">
                        <input
                          ...
                          className={`ae-theme-field h-12 w-20 rounded-xl border ${item.borderColor} px-3 text-center text-xl font-bold transition-all focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:ring-offset-0`}
                        />
                      </div>
                    </div>
                  </div>
            ...
            {/* Footer */}
            <div className="ae-theme-inset relative border-t px-6 py-5">
              <div className="flex items-center gap-3">
                <button
                  onClick={handleClose}
                  className="ae-theme-button-muted flex-1 rounded-xl border px-5 py-3 text-sm font-medium transition-all duration-200"
                >
                  Cancel
                </button>
                <button
