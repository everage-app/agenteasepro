...
$emailStatus = $resp.emailStatus
$expectedEmails = $links.Count
$sentEmails = if ($emailStatus) { [int]$emailStatus.sent } else { 0 }
$failedEmails = if ($emailStatus) { [int]$emailStatus.failed } else { 0 }
$firstEmailFailure = $null

if ($emailStatus -and $emailStatus.results) {
  $firstEmailFailure = $emailStatus.results | Where-Object { -not $_.success } | Select-Object -First 1
}

if ($failedEmails -gt 0) {
  $detail = if ($firstEmailFailure -and $firstEmailFailure.error) { " First error: $($firstEmailFailure.error)." } else { '' }
  $message = "Envelope created, but $failedEmails of $expectedEmails email(s) failed.$detail Set SENDGRID_API_KEY in the root .env (preferred) or server/.env, confirm SENDGRID_FROM_EMAIL, then restart the API server."
  if ($RequireEmails) {
    throw $message
  }
  Write-Warning $message
} elseif ($RequireEmails -and (-not $emailStatus -or $sentEmails -lt $expectedEmails)) {
  throw "Envelope created, but email delivery status was incomplete ($sentEmails of $expectedEmails sent). Restart the API server after adding SendGrid env and rerun."
}
